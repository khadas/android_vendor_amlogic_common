...
#define HCI_VSC_MASTER_LATENCY                  0xFDC7
...
#define MASTER_LATENCY_CMD_PARAM_SIZE           6
...
void hw_master_latency_ctrl_cback(void *p_mem)
{
...
uint8_t hw_set_master_latency_default_value(uint16_t master_latency, uint16_t master_latency_timeout)
{
    HC_BT_HDR  *p_buf = NULL;
    uint8_t     *p;
    uint8_t     ret = FALSE;
    ...
    if (p_buf)
    {
        ...
        p_buf->offset = 0;
        p_buf->layer_specific = 0;
        p_buf->len = HCI_CMD_PREAMBLE_SIZE + MASTER_LATENCY_CMD_PARAM_SIZE;

        p = (uint8_t *) (p_buf + 1);
        UINT16_TO_STREAM(p, HCI_VSC_MASTER_LATENCY);
        *p++ = MASTER_LATENCY_CMD_PARAM_SIZE; /* parameter length */
        UINT16_TO_STREAM(p, 0x0fff); //handle to change for default settings
        UINT16_TO_STREAM(p, master_latency); //master latency
        UINT16_TO_STREAM(p, master_latency_timeout); //master latency timeout

        if ((ret = bt_vendor_cbacks->xmit_cb(HCI_VSC_MASTER_LATENCY, p_buf, \
                                        hw_master_latency_ctrl_cback)) == FALSE)
        {
            bt_vendor_cbacks->dealloc(p_buf);
        }
    }
...
uint8_t IsSupportWOLE(void)
{
    fw_support_wole_entry_t *p_entry;
    p_entry = (fw_support_wole_entry_t *)fw_support_wole_table;
    while (p_entry->chip_id != NULL)
    {
        ALOGE("Chipset %s, p_entry = %s", hw_cfg_cb.local_chip_name,p_entry->chip_id);
        if ((strstr(hw_cfg_cb.local_chip_name, p_entry->chip_id) != NULL))
            return 1;
        p_entry++;
    }
    return 0;
...
uint8_t hw_check_chip_name (void)
{
    int chip_name_flag;
    ALOGE("Chipset %s", hw_cfg_cb.local_chip_name);
    if ((strstr(hw_cfg_cb.local_chip_name, "BCM4359C0") != NULL) || (strstr(hw_cfg_cb.local_chip_name, "BCM4354A2") != NULL))
        chip_name_flag = 1;
    else
        chip_name_flag = 0;

    return chip_name_flag;
