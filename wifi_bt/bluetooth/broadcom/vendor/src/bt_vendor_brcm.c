...
static const char* MODULE_LIBRARY_NAME = "";
...
static int init(const bt_vendor_callbacks_t* p_cb, unsigned char *local_bdaddr)
{
    ...
    userial_vendor_init();
    upio_init();
    property_get("persist.vendor.bt_module", bt_module_name_temp, MODULE_LIBRARY_NAME);
    ALOGI("get bt_module %s",bt_module_name_temp);

    if (strcmp(bt_module_name_temp,"btusb"))
    {
        ALOGI("load uart conf");
        vnd_load_conf(VENDOR_LIB_CONF_FILE);
    }
    else
    {
        ALOGI("load uab conf");
        vnd_load_conf("/vendor/etc/bluetooth/bt_vendor_usb.conf");
    }
        ...
        case BT_VND_OP_LPM_SET_MODE:
            {
                ...
                //retval = hw_set_master_latency_default_value(1,0xf);
                if (IsSupportWOLE()) {
