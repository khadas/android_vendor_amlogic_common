...
extern int qca_woble_configure(int fd);
/******************************************************************************
**  Variables
******************************************************************************/
int pFd[2] = {0,};
		...
		case BT_SOC_ROME:
		{
			...
			if (fd < 0) {
				...
				if (rome_soc_init(fd, (char *)vnd_local_bd_addr) < 0) {
                                    ALOGE("%s: Use 2M baudrate to reinitialize.\n", __func__);
                                    fd = userial_vendor_open((tUSERIAL_CFG *) &userial_cfg);
                                    if (fd < 0) {
                                        ALOGE("userial_vendor_open returns err");
                                    } else {
                                        int err;
...
uart_fd = fd;                                                   
	...
	{
		ALOGE("bt-vendor : BT_VND_OP_USERIAL_CLOSE btSocType: %d", btSocType);
		switch (btSocType)
		{
		case BT_SOC_DEFAULT:
		...
		case BT_SOC_AR3K:
			property_set("vendor.wc_transport.clean_up", "1");
			qca_woble_configure(vnd_userial.fd);
