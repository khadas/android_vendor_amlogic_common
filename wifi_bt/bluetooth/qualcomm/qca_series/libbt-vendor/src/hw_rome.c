...
if (ptlv_header->tlv_type == TLV_TYPE_PATCH) {
			...
			nvm_index < nvm_length ; nvm_ptr = (tlv_nvm_hdr *) nvm_byte_ptr)
	{
		...
		if (nvm_ptr->tag_id == TAG_NUM_2) {
			ALOGI("skip copying BD Address from host side. Use NVM address.");
...
ALOGI("%s: Downloading TLV Patch segment no.%d, size:%d, wait_cc_evt:%d", __FUNCTION__, index, seg_size, wait_cc_evt);
...
for (i = 0; i < total_segment ; i++) {
	if ((i + 1) == total_segment) {
		if ((rom >= ROME_PATCH_VER_0101) && (rom < ROME_PATCH_VER_0302) && (gTlv_type == TLV_TYPE_PATCH)) {
		...
		} else if (((rom == ROME_PATCH_VER_0302) || (rom == NAPLES_PATCH_VER_0100)) && (gTlv_type == TLV_TYPE_PATCH)) {
...
if ( (rom >= ROME_PATCH_VER_0101) && (rom < ROME_PATCH_VER_0302) && (gTlv_type == TLV_TYPE_PATCH)) {
	 ...
	 */
	wait_cc_evt = remain_size ? FALSE : TRUE;
} else if ((rome_ver == NAPLES_VER_1_0) && (gTlv_type == TLV_TYPE_PATCH)) {
	/* If the Chip version is Naples 1.0
	 * 1. None of the command segments receive CCE
	 * 2. No command segments receive VSE except the last one
	 * 3. If gTlv_dwndCfg is ROME_SKIP_EVT_NONE then the logic is
	 *    same as Rome 2.1, 2.2, 3.0
...
cmd[4]  = BAUDRATE_2000000; //amlogic only support 2M

/* Total length of the packet to be sent to the Controller */
size = (HCI_CMD_IND + HCI_COMMAND_HDR_SIZE + VSC_SET_BAUDRATE_REQ_LEN);
tcflush(fd, TCIOFLUSH);
ALOGD("delay 20ms after tcflush...");
usleep(20 * 1000);
...
/* Send one more change baud cmd to ensure both module & host are on 2M baud*/
err = do_write(fd, cmd, size);
if (err != size) {
