...
static const char VENDOR_AMLBTVER_PROPERTY[] =
	"vendor.sys.amlbtversion";
...
enum {
	...
	HW_CFG_SET_BD_ADDR,
	HW_CFG_SET_PARAMS,
	HW_CFG_AML_UPDATE_BAUDRATE,
	HW_CFG_AML_UPDATE_BAUDRATE_UART,
	HW_CFG_AML_DOWNLOAD_FIRMWARE_ICCM_UART,
	HW_CFG_AML_DOWNLOAD_FIRMWARE_DCCM_UART,
	HW_CFG_AML_DOWNLOAD_FIRMWARE_CHANGE_BAUDRATE_UART,
	HW_CFG_AML_DOWNLOAD_FIRMWARE_CLOSE_EVENT,
	HW_CFG_AML_CONFIG_RF_CALIBRATION,
	HW_CFG_AML_DOWNLOAD_FIRMWARE_STRAT_CPU_UART_BEFORE,
	HW_CFG_AML_DOWNLOAD_FIRMWARE_STRAT_CPU_UART,
	HW_CFG_AML_DOWNLOAD_FIRMWARE_FINISH_UART,
	HW_CFG_AML_DOWNLOAD_FIRMWARE_TEST_UART,
	HW_CFG_AML_DOWNLOAD_FIRMWARE_TEST_2_UART,
	HW_CFG_AML_DOWNLOAD_FIRMWARE_TEST_3_UART,
	HW_CFG_AML_SET_BAUDRATE
...
static uint8_t hw_config_set_rf_params(HC_BT_HDR *p_buf)
{
	...
	BTHWDBG("Setting parameters to controller: antenna number=%d.", antenna_num);

	UINT16_TO_STREAM(p, TCI_WRITE_REG);
	*p++ = 8;                       /* parameter length */
	UINT32_TO_STREAM(p, 0xf03040);  /* addr */
	if (antenna_num == 1)
	{
...
void hw_config_cback(void *p_mem)
{
	...
	if (p_buf != NULL)
	{
...
#ifdef AML_DOWNLOADFW_UART
			p_name = "/etc/bluetooth/bt_fucode.h";
			if (1)
#else
...
#ifdef AML_DOWNLOADFW_UART
		case HW_CFG_AML_DOWNLOAD_FIRMWARE_TEST_UART:
			p_tmp = p_name = (char *)(p_evt_buf + 1) + \
					 HCI_EVT_CMD_CMPL_LOCAL_NAME_STRING;
			...
			*p++ = 4;                               /* parameter length */
			...
			break;
		case HW_CFG_AML_DOWNLOAD_FIRMWARE_TEST_2_UART:
			UINT16_TO_STREAM(p, TCI_READ_REG);
			*p++ = 4;                       /* parameter length */
			UINT32_TO_STREAM(p, 0xf03050);  /* addr */
		...
		case HW_CFG_AML_DOWNLOAD_FIRMWARE_TEST_3_UART:
			UINT16_TO_STREAM(p, TCI_WRITE_REG);
			*p++ = 8;                       /* parameter length */
			UINT32_TO_STREAM(p, 0xf03050);  /* addr */
		...
		case HW_CFG_AML_UPDATE_BAUDRATE_UART:
			userial_vendor_set_baud( \
			...
			*p++ = 8;
			...
			break;
		case HW_CFG_AML_DOWNLOAD_FIRMWARE_ICCM_UART:
			if (len_iccm <= 0) {
			...
			*p++ = cmd_len_iccm; /* parameter length */
			...
			{
				p[i] = *(BT_fwICCM + offset_iccm + i);/* data */
			}
		...
		case HW_CFG_AML_DOWNLOAD_FIRMWARE_DCCM_UART:
			if (len_dccm <= 0) {
		...
		case HW_CFG_AML_DOWNLOAD_FIRMWARE_CHANGE_BAUDRATE_UART:
			...
			*p++ = 8;
			...
			is_proceeding = bt_vendor_cbacks->xmit_cb(TCI_UPDATE_UART_BAUDRATE, \
								  p_buf, hw_config_cback);
			break;
		case HW_CFG_AML_DOWNLOAD_FIRMWARE_CLOSE_EVENT:
			UINT16_TO_STREAM(p, TCI_WRITE_REG);
			*p++ = 8;
			UINT32_TO_STREAM(p, 0xa70014);
		...
		case HW_CFG_AML_CONFIG_RF_CALIBRATION:
			if ((is_proceeding = hw_config_set_rf_params(p_buf)) == TRUE)
				break;
		...
		case HW_CFG_AML_DOWNLOAD_FIRMWARE_STRAT_CPU_UART_BEFORE:
			UINT16_TO_STREAM(p, TCI_WRITE_REG);
			*p++ = 8;
		...
		case HW_CFG_AML_DOWNLOAD_FIRMWARE_STRAT_CPU_UART:
			UINT16_TO_STREAM(p, TCI_UPDATE_UART_BAUDRATE);
			*p++ = 8;
			UINT32_TO_STREAM(p, 0xf03058);
			UINT32_TO_STREAM(p, 0x700);
			...
			break;
		case HW_CFG_AML_DOWNLOAD_FIRMWARE_FINISH_UART:
			...
			break;
#endif
		case HW_CFG_AML_UPDATE_BAUDRATE:
		...
		case HW_CFG_AML_SET_BAUDRATE:
			break;
		case HW_CFG_SET_PARAMS:
			ms_delay(350);
		...
		case HW_CFG_SET_BD_ADDR:
			...
			BTHWDBG("BT Controller Version,data = %02x.%02x,number = 0x%02x%02x", *(p_tmp + 1), *p_tmp, *(p_tmp + 3), *(p_tmp + 2));
			sprintf(local_ver, "data = %02x.%02x,number = 0x%02x%02x", *(p_tmp + 1), *p_tmp,*(p_tmp + 3), *(p_tmp + 2));
