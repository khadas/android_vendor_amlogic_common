...
ota_patch_t ota_patch_info = {0, -1, PTHREAD_MUTEX_INITIALIZER, PTHREAD_COND_INITIALIZER, 0, NULL};
/******************************************************************************
**  Static functions
******************************************************************************/
...
static queue_cmd_cb_t queue_cmd_cb;
...
void userial_vendor_init(char *bt_device_node)
{
    ...
    vnd_userial.cmd_data = RtbQueueInit();

    sem_init(&queue_cmd_cb.cmd_queue_sem, 0, 0);
    sem_init(&queue_cmd_cb.cmd_event_sem, 0, 1);
...
static void userial_socket_close(void)
{
    ...
    sem_post(&queue_cmd_cb.cmd_queue_sem);
    sem_post(&queue_cmd_cb.cmd_event_sem);
    if (queue_cmd_cb.thread_cmd_send_id != -1)
    {
        if ((result = pthread_join(queue_cmd_cb.thread_cmd_send_id, NULL)) < 0)
        {
            ALOGE("pthread_join() queue_cmd_cb.thread_cmd_send_id failed result:%d", result);
        }
        else
        {
            queue_cmd_cb.thread_cmd_send_id = -1;
            ALOGE("pthread_join() queue_cmd_cb.thread_cmd_send_id pthread_join_success result:%d", result);
        }
    }
...
void userial_vendor_close(void)
{
    ...
    sem_destroy(&queue_cmd_cb.cmd_queue_sem);
    sem_destroy(&queue_cmd_cb.cmd_event_sem);
#ifdef RTK_INQUIRY_PAGESCAN
...
#endif
    if (ota_patch_info.thread_ota_dl_patch_running)
    {
        ota_patch_info.thread_ota_dl_patch_running = false;
        pthread_mutex_lock(&(ota_patch_info.ota_mutex));
        pthread_cond_signal(&(ota_patch_info.ota_cond));
        pthread_mutex_unlock(&(ota_patch_info.ota_mutex));
        if (ota_patch_info.thread_ota_dl_patch_id != -1)
        {
            if (pthread_join(ota_patch_info.thread_ota_dl_patch_id, NULL) != 0)
            {
                ALOGE("%s ota_patch_info.thread_ota_dl_patch_id  pthread_join_failed", __func__);
            }
            else
            {
                ota_patch_info.thread_ota_dl_patch_id = -1;
            }
        }
    }
...
static void userial_send_iso_to_controller(unsigned char *recv_buffer, int total_length)
{
    ...
    {
        h5_int_interface->h5_send_acl_data(DATA_TYPE_ISO, &recv_buffer[1], (total_length - 1));
    }
    userial_enqueue_coex_rawdata(recv_buffer, total_length, false);
}
...
static int userial_coex_recv_data_handler(unsigned char *recv_buffer, int total_length)
{
    ...
    case RTKBT_PACKET_HEADER:
        ...
        if (coex_current_type == DATA_TYPE_ACL)
        {
            STREAM_TO_UINT16_S(coex_packet_bytes_need, (coex_resvered_buffer + 2));
        }
        else if (coex_current_type == DATA_TYPE_EVENT)
        {
    ...
    case RTKBT_PACKET_END:
        {
            ...
            case DATA_TYPE_ACL:
                ...
                STREAM_TO_UINT16_S(handle, coex_resvered_buffer);
            ...
            else
            {
                ...
                if (hci_firmware_log_fd != INVALID_FD)
                {
                    hci_log_firmware_debug_packet_rtk(hci_firmware_log_fd, p_buf, coex_current_type);
                }
            }
        }
        break;
...
static void userial_coex_send_data_handler(unsigned char *send_buffer, int total_length)
{
    ...
    case DATA_TYPE_ACL:
        ...
        STREAM_TO_UINT16_S(handle, (send_buffer + 1));
        boundary_flag = RTK_GET_BOUNDARY_FLAG(handle);
        if (rtk_parse_manager)
        {
...
static bool userial_enqueue_cmd(uint8_t *cmd, uint16_t total_length, uint8_t is_libbtvendor_cmd,
                                tINT_CMD_CBACK p_cback)
{
    RTK_BUFFER *skb_cmd = RtbAllocate(total_length, 0);
    if (skb_cmd == NULL)
    {
        ALOGE("%s RtbAllocate failed", __func__);
        return false;
    }
    memcpy(skb_cmd->Data, cmd, total_length);
    skb_cmd->Length = total_length;
    skb_cmd->BtVendorCmdFlag = is_libbtvendor_cmd;
    skb_cmd->Pcback = p_cback;
    RtbQueueTail(vnd_userial.cmd_data, skb_cmd);
    sem_post(&queue_cmd_cb.cmd_queue_sem);
    return true;
}

static void *userial_sent_cmd_thread()
{
    RTK_BUFFER *skb_cmd;
    uint16_t opcode;
    while (vnd_userial.thread_running)
    {
        ...
        sem_wait(&queue_cmd_cb.cmd_event_sem);
...
//all cmd sent by libbt-vendor must call this api func
bool userial_vendor_send_cmd_to_controller(unsigned char *recv_buffer, int total_length,
                                           tINT_CMD_CBACK p_cback)
{
    return userial_enqueue_cmd(recv_buffer, total_length, 1, p_cback);
}
    ...
    */
    ...
    if (!sco_cb.msbc_used)
    {
        ...
        for (i = 0; i < 6; i++)
        {
            ...
            send_data[3] = 48;
            UINT16_TO_STREAM_S((send_data + 1), sco_cb.sco_handle);
        ...
        else
        {
            ...
            for(i = 0; i < 5; i ++) {
                if(fread(&enc_data[4], 1, 48, file) > 0) {
                    ...
                    enc_data[3] = 48;
                    UINT16_TO_STREAM_S((enc_data + 1), sco_cb.sco_handle);
            ...
            */
            ...
            for (i = 0; i < 4; i++)
            {
                ...
                {
                    UINT16_TO_STREAM_S((enc_data + i * 60), btui_msbc_h2[sco_cb.pcm_enc_seq % 4]);
                    sco_cb.pcm_enc_seq++;
...
static int userial_handle_cmd(unsigned char *recv_buffer, int *total_length)
{
...
#endif
            STREAM_TO_UINT16_S(scan_int, (recv_buffer + 4));
            STREAM_TO_UINT16_S(scan_win, (recv_buffer + 6));
            if (scan_win > 20)
            {
                if ((scan_int / scan_win) > 2)
                {
                    UINT16_TO_STREAM_S((recv_buffer + 4), ((scan_int * 20) / scan_win));
                    UINT16_TO_STREAM_S((recv_buffer + 6), 20);
                }
                else
                {
                    UINT16_TO_STREAM_S((recv_buffer + 4), 40);
                    UINT16_TO_STREAM_S((recv_buffer + 6), 20);
                }
            }
            else if (scan_win == scan_int)
            {
                UINT16_TO_STREAM_S((recv_buffer + 4), ((scan_int * 5) & 0x00FE));
            }
            else if ((scan_int / scan_win) <= 2)
            {
                UINT16_TO_STREAM_S((recv_buffer + 4), ((scan_int * 3) & 0x00FE));
            }
        }
        break;
...
#endif
            STREAM_TO_UINT16_S(scan_int, (recv_buffer + 7));
            STREAM_TO_UINT16_S(scan_win, (recv_buffer + 9));
            if (scan_win > 20)
            {
                if ((scan_int / scan_win) > 2)
                {
                    UINT16_TO_STREAM_S((recv_buffer + 7), ((scan_int * 20) / scan_win));
                    UINT16_TO_STREAM_S((recv_buffer + 9), 20);
                }
                else
                {
                    UINT16_TO_STREAM_S((recv_buffer + 7), 80);
                    UINT16_TO_STREAM_S((recv_buffer + 9), 40);
                }
            }
            else if (scan_win == scan_int)
            {
                UINT16_TO_STREAM_S((recv_buffer + 7), ((scan_int * 5) & 0x00FE));
            }
            else if ((scan_int / scan_win) <= 2)
            {
                UINT16_TO_STREAM_S((recv_buffer + 7), ((scan_int * 3) & 0x00FE));
            }
            if (is_cts_presence_test)
            {
                UINT16_TO_STREAM_S((recv_buffer + 7), 1600);
                UINT16_TO_STREAM_S((recv_buffer + 9), 1552);
    ...
    case HCI_LE_SET_CIG_PARAMS:
        {
            uint16_t tmp;
            STREAM_TO_UINT16_S(tmp, (recv_buffer + 19));
            ...
            STREAM_TO_UINT16_S(tmp, (recv_buffer + 21));
            ...
            uint8_t gen_cis_iso_num_pkt_cmd[7] = {0x01, 0xBD, 0xFD, 0x03, 0x0B, 0x09, 0x01};
            userial_vendor_send_cmd_to_controller(gen_cis_iso_num_pkt_cmd, 7, NULL);
        }
        break;
    ...
    case HCI_WRITE_VOICE_SETTINGS:
        {
            STREAM_TO_UINT16_S(voice_settings, (recv_buffer + 3));
    ...
    case HCI_SET_EVENT_MASK:
        {
            ...
            if (rtkbt_transtype & RTKBT_TRANS_USB)
            {
                uint8_t read_iso_handle_range_cmd[4];
                uint8_t *p = read_iso_handle_range_cmd;
                UINT8_TO_STREAM(p, DATA_TYPE_COMMAND);
                UINT16_TO_STREAM(p, HCI_VENDOR_READ_ISO_HANDLE_RANGE);
                *p = 0;
                userial_vendor_send_cmd_to_controller(read_iso_handle_range_cmd, HCI_CMD_MIN_SIZE, NULL);
            }
            if (vnd_userial.thread_running)
            {
                Heartbeat_init();
            }
        }
    ...
    case HCI_BLE_CREATE_LL_CONN:
        {
                 ...
                 (rtkbt_adv_con.adverting_type == 0x04)))
            {
                ...
                rtkbt_adv_con.adverting_enable = FALSE;
                userial_vendor_send_cmd_to_controller(disable_adv_cmd, 5, NULL);
            }

            STREAM_TO_UINT16_S(scan_int, (recv_buffer + 3));
            STREAM_TO_UINT16_S(scan_win, (recv_buffer + 5));
            if ((scan_int / scan_win) > (BLE_SCAN_SLOW_INT_1 / BLE_SCAN_SLOW_WIN_1))
            {
                UINT16_TO_STREAM_S((recv_buffer + 3), BLE_SCAN_SLOW_INT_1);
                UINT16_TO_STREAM_S((recv_buffer + 5), BLE_SCAN_SLOW_WIN_1);
            }

            UINT16_TO_STREAM_S((recv_buffer + 16), BLE_CONN_INT_MIN_DEF);
            UINT16_TO_STREAM_S((recv_buffer + 18), BLE_CONN_INT_MAX_DEF);
        }
        break;

    case HCI_LE_EXTENDED_CREATE_CONNECTION :
        {
            STREAM_TO_UINT16_S(scan_int, (recv_buffer + 13));
            STREAM_TO_UINT16_S(scan_win, (recv_buffer + 15));
            if ((scan_int / scan_win) > (BLE_SCAN_SLOW_INT_1 / BLE_SCAN_SLOW_WIN_1))
            {
                UINT16_TO_STREAM_S((recv_buffer + 13), BLE_SCAN_SLOW_INT_1);
                UINT16_TO_STREAM_S((recv_buffer + 15), BLE_SCAN_SLOW_WIN_1);

                UINT16_TO_STREAM_S((recv_buffer + 29), BLE_SCAN_SLOW_INT_1);
                UINT16_TO_STREAM_S((recv_buffer + 31), BLE_SCAN_SLOW_WIN_1);

                UINT16_TO_STREAM_S((recv_buffer + 45), BLE_SCAN_SLOW_INT_1);
                UINT16_TO_STREAM_S((recv_buffer + 47), BLE_SCAN_SLOW_WIN_1);

            }

            UINT16_TO_STREAM_S((recv_buffer + 17), BLE_CONN_INT_MIN_DEF);
            UINT16_TO_STREAM_S((recv_buffer + 19), BLE_CONN_INT_MAX_DEF);

            UINT16_TO_STREAM_S((recv_buffer + 33), BLE_CONN_INT_MIN_DEF);
            UINT16_TO_STREAM_S((recv_buffer + 35), BLE_CONN_INT_MAX_DEF);

            UINT16_TO_STREAM_S((recv_buffer + 49), BLE_CONN_INT_MIN_DEF);
            UINT16_TO_STREAM_S((recv_buffer + 51), BLE_CONN_INT_MAX_DEF);
        }
        break;
    ...
    case HCI_BLE_ADV_FILTER_OCF:
        {
            if (recv_buffer[3] == 0x07 && recv_buffer[4] == 0x00) //APCF Service Data Add
            {
                uint8_t uuid_cts[] = {0x63, 0xbd, 0x2d, 0x50, 0x90, 0xc0, 0x47, 0x8e, 0x4d, 0x4d, 0xf1, 0x73, 0x0d, 0x95, 0xb7, 0xcd};
                uint8_t *data_master = recv_buffer + 22;
                if (!memcmp(data_master, uuid_cts, 16))
                {
                    filter_index = recv_buffer[5];
                }
            }
            if (recv_buffer[3] == 0x01 && recv_buffer[4] == 0x00) //APCF Set Filtering parameters Add
            {
                if (recv_buffer[5] == filter_index)
                {
                    is_cts_presence_test = TRUE;
                    ALOGI("is_cts_presence_test start");
                }
            }
            if (recv_buffer[3] == 0x01 && recv_buffer[4] == 0x01) //APCF Set Filtering parameters Deleted
            {
                if (recv_buffer[5] == filter_index)
                {
                    is_cts_presence_test = FALSE;
                    ALOGI("is_cts_presence_test finish");
                }
            }
        }
        break;
    ...
    case HCI_WRITE_SCAN_ENABLE:
        {
            if (recv_buffer[3] != 0 && !br_scan_flag)
            {
                br_scan_flag = true;
            }
            else if (recv_buffer[3] == 0 && br_scan_flag)
            {
                br_scan_flag = false;
            }
            ALOGD("@roc HCI_WRITE_SCAN_ENABLE, br_scan_flag: %d", br_scan_flag);
        }
        break;

    case HCI_INQUIRY:
        {
            if (br_scan_flag && inquiry_count == 0)
            {
            ...
            else if (br_scan_flag && inquiry_count)
            {
                inquiry_count --;
                recv_buffer[6] = VND_INQUIRY_LENGTH;
            }
...
static void userial_recv_H4_rawdata(void *context)
{
    ...
    case RTKBT_PACKET_HEADER:
        ...
        {
            STREAM_TO_UINT16_S(packet_bytes_need, (h4_read_buffer + COMMON_DATA_LENGTH_INDEX));
        }
    ...
    case RTKBT_PACKET_END:
        ...
        case DATA_TYPE_COMMAND:
            ...
            // {
            //     STREAM_TO_UINT16_S(opcode, (h4_read_buffer + 1));
            //     if (opcode == HCI_VSC_H5_INIT)
            //     {
            ...
            userial_enqueue_cmd(h4_read_buffer, (h4_read_length + 1), 0, NULL);
            break;
...
static void userial_vnd_inquiry_again(union sigval sigev_value)
{
    ...
    uint8_t cmd[9] = {0x01, 0x01, 0x04, 0x05, 0x33, 0x8B, 0x9E, VND_INQUIRY_LENGTH, 0x00};
    userial_vendor_send_cmd_to_controller(cmd, 9, NULL);
}
    ...
    case HCI_COMMAND_COMPLETE_EVT:
        {
            ...
            else if (opcode == HCI_LE_SET_EXTENDED_SCAN_ENABLE)
            {
            ...
            else if (opcode == HCI_BLE_VENDOR_CAP_OCF)
            ...
            {
                if (p_data[5] == 0 && p_data[11] == 1 &&
                    (p_data[12] - 3 >= pwr_cfg.nm_filter_idx)) //status:0 && support for filtering in the controller:1
                {
                    pwr_cfg.start_filter_idx = p_data[12] - pwr_cfg.nm_filter_idx;
                    p_data[12] = p_data[12] - pwr_cfg.nm_filter_idx; //save index for config wake up function
                }
            }
            else if (opcode == HCI_VSC_DOWNLOAD_FW_PATCH)
            {
                if (ota_patch_info.thread_ota_dl_patch_id != -1)
                {
                    pthread_mutex_lock(&(ota_patch_info.ota_mutex));
            ...
            else
            {
                ...
                if (skb_cmd)
                {
                    ...
                    {
                        sem_post(&queue_cmd_cb.cmd_event_sem);
                        if (skb_cmd->BtVendorCmdFlag)//libbt-vendor cmd cmplt evt
                        {
                            if (skb_cmd->Pcback)
                            {
            ...
            else
            {
                ...
                if (skb_cmd)
                {
                    ...
                    if (skb_cmd_opcode == opcode)
                    {
                        ...
                        {
                            uint8_t status = recv_buffer[2];
                            if (vnd_inquiry_flag)
                            {
                                vnd_inquiry_flag = false;
                                if (status != 0)
                                {
                                    ALOGD("@roc HCI_INQUIRY cmd status: %d", status);
                                    inquiry_page_status &= ~VND_STATUS_INQUIRY;
                                    if (inquiry_count != 0)
                                    {
                                    ...
                                    {
                                        uint8_t p_buf[7] = {0x04, 0x01, 0x01, 0x00};
                                        userial_recv_rawdata_hook(p_buf, 4);
                                    }
                                }
                            }
                            else
    ...
    case HCI_BLE_EVENT:
        {
            ...
            else if (subcode == HCI_LE_EXTENDED_ADVERTISING_REPORT_EVT)
            {
                r_cn = *(uint8_t *)&p_data[3];//Num_Reports
                p_data = p_data + 4;//reports data
                for (i = 0; i < r_cn ; i++)
            ...
            else if ((subcode == HCI_BLE_CONN_COMPLETE_EVT) || (subcode == HCI_BLE_ENHANCED_CONN_COMPLETE_EVT))
            {
...
#ifdef VENDOR_MESH_RTK
            else if (subcode == HCI_LE_ADVERTISING_SET_TERMINATED_EVT)
            {
                if (0xEF == p_data[4]) // now we assume adv hdl 0xef is mesh adv hdl
                {
                    return 1;
                }
            }
...
static int userial_handle_recv_data(unsigned char *recv_buffer, unsigned int total_length)
{
    ...
    case RTKBT_PACKET_HEADER:
        ...
        {
            STREAM_TO_UINT16_S(received_packet_bytes_need, (received_resvered_header + 2));
        }
    ...
    STREAM_TO_UINT16_S(len, received_resvered_data + 3);
    if ((received_resvered_data[0] == DATA_TYPE_ACL) && (len == 0))
    {
...
static void *userial_recv_socket_thread(void *arg)
{
    ...
    while (vnd_userial.thread_running)
    {
        ...
        do
        {
            ret = epoll_wait(vnd_userial.epoll_fd, events, 32, 200);
        }
...
void ota_download_fw_patch(uint8_t index, uint8_t *data, int len)
{
    ALOGD("%s idx:%i", __func__, index);
    uint8_t download_fw_cmd[257] = {0x01, 0x20, 0xFC};
    download_fw_cmd[3] = len + 1;
    download_fw_cmd[4] = index;
    memcpy(download_fw_cmd + 5, data, len);
    userial_vendor_send_cmd_to_controller(download_fw_cmd, HCI_CMD_MIN_SIZE + len + 1, NULL);
}

void *ota_update_fw_patch_thread(void *arg)
{
    RTK_UNUSED(arg);
    ALOGD("%s sleep 600ms", __func__);
    usleep(600000);
    uint16_t patch_frag_cnt = 0;
    uint8_t patch_frag_tail = 0;
    uint16_t patch_frag_idx = 0;
    uint8_t idxRx = 0;
    ota_patch_info.thread_ota_dl_patch_running = true;
    if (ota_patch_info.p_ota_patch && ota_patch_info.ota_patch_len)
    {
        patch_frag_cnt = ota_patch_info.ota_patch_len / PATCH_DATA_FIELD_MAX_SIZE;
        patch_frag_tail = ota_patch_info.ota_patch_len % PATCH_DATA_FIELD_MAX_SIZE;
        if (patch_frag_tail)
        {
            patch_frag_cnt += 1;
        }
        ...
        {
            patch_frag_tail = PATCH_DATA_FIELD_MAX_SIZE;
        }
        ALOGD("%s ota patch fragment count %d, tail len %d", __func__, patch_frag_cnt, patch_frag_tail);
        while (ota_patch_info.thread_ota_dl_patch_running && (patch_frag_idx < patch_frag_cnt))
        {
            pthread_mutex_lock(&(ota_patch_info.ota_mutex));
            if (patch_frag_idx == patch_frag_cnt - 1)
            {
                ALOGD("OTA update: send last fw fragment");
                idxRx |= 0x80;
                ota_download_fw_patch(idxRx, ota_patch_info.p_ota_patch + (patch_frag_idx *
                                                                           PATCH_DATA_FIELD_MAX_SIZE), patch_frag_tail);
            }
            else
            {
                idxRx &= 0x7F;
                ota_download_fw_patch(idxRx, ota_patch_info.p_ota_patch + (patch_frag_idx *
                                                                           PATCH_DATA_FIELD_MAX_SIZE), PATCH_DATA_FIELD_MAX_SIZE);
            }
            patch_frag_idx += 1;
            idxRx = (idxRx + 1) % 0x80;
            pthread_cond_wait(&(ota_patch_info.ota_cond), &(ota_patch_info.ota_mutex));
            pthread_mutex_unlock(&(ota_patch_info.ota_mutex));
        }
        ota_patch_info.ota_patch_len = 0;
        free(ota_patch_info.p_ota_patch);
        ota_patch_info.p_ota_patch = NULL;
    }
    ALOGD("%s exit", __func__);
    return NULL;
}

void check_fw_update_cmd_complete_cback(void *arg)
{
    uint8_t *p_buf = (uint8_t *)arg;
    uint8_t ota = 0;

    ALOGD("%s", __func__);
    if (arg != NULL)
    {
        ota = *(p_buf + 7); // ota offset is 7
        ALOGD("%s ota:%d", __func__, ota);
        if (ota == 1)
        {
            if (pthread_create(&(ota_patch_info.thread_ota_dl_patch_id), NULL, ota_update_fw_patch_thread,
                               NULL) != 0)
            {
                ALOGE("%s create thread failed: %s", __func__, strerror(errno));
                ota_patch_info.thread_ota_dl_patch_id = -1;
                if (ota_patch_info.p_ota_patch)
                {
                    free(ota_patch_info.p_ota_patch);
                    ota_patch_info.p_ota_patch = NULL;
                }
...
static void *userial_coex_thread(void *arg)
{
    ...
    while (vnd_userial.thread_running)
    {
        ...
        do
        {
            ret = epoll_wait(vnd_userial.cpoll_fd, events, 64, 200);
        }
        while (ret == -1 && errno == EINTR && vnd_userial.thread_running);
        if (ret == -1)
        {
...
int userial_socket_open()
{
    ...
    if (pthread_getschedparam(vnd_userial.thread_socket_id, &policy, &param) == 0)
    {
        policy = SCHED_FIFO;
        param.sched_priority  = 99;
        result = pthread_setschedparam(vnd_userial.thread_socket_id, policy, &param);
        if (result != 0)
        {
    ...
    if (pthread_getschedparam(vnd_userial.thread_uart_id, &policy, &param) == 0)
    {
        policy = SCHED_FIFO;
        param.sched_priority  = 99;
        result = pthread_setschedparam(vnd_userial.thread_uart_id, policy, &param);
        if (result != 0)
        {
            ALOGE("%s : pthread_setschedparam failed (%s)", __func__,
                  strerror(result));
    ...
    if (pthread_create(&queue_cmd_cb.thread_cmd_send_id, &thread_attr, userial_sent_cmd_thread,
                       NULL) != 0)
    {
        ALOGE("pthread_create: %s", strerror(errno));
        queue_cmd_cb.thread_cmd_send_id = -1;
    ...
    if (pthread_getschedparam(queue_cmd_cb.thread_cmd_send_id, &policy, &param) == 0)
    {
        policy = SCHED_FIFO;
        param.sched_priority  = 99;
        result = pthread_setschedparam(queue_cmd_cb.thread_cmd_send_id, policy, &param);
        if (result != 0)
        {
            ALOGE("%s : pthread_setschedparam failed (%s)", __func__,
                  strerror(result));
