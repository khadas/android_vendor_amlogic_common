...
//  {0x8761,            HCI_VERSION_MASK_ALL,    HCI_REVISION_MASK_ALL, CHIP_TYPE_MASK_ALL,  1<<3,                  "rtl8761at_fw",         "rtl8761at_config",     CONFIG_MAC_OFFSET_GEN_1_2,  MAX_PATCH_SIZE_24K},     //Rtl8761AW
    {0x8761,            HCI_VERSION_MASK_42_BELOW,    ~(1 << 0xb),             CHIP_TYPE_MASK_ALL,  1 << 3,                  "rtl8761at_fw",         "rtl8761at_config",     CONFIG_MAC_OFFSET_GEN_1_2,  MAX_PATCH_SIZE_24K}, //Rtl8761AW
    ...
    {0x8761,            HCI_VERSION_MASK_ALL, (1 << 0xe),              CHIP_TYPE_MASK_ALL,  1ULL << 51,                 "rtl8761ct_mx_fw",         "rtl8761ct_mx_config",     CONFIG_MAC_OFFSET_GEN_4PLUS,  MAX_PATCH_SIZE_500k},  //Rtl8761CT_mx
...
static int check_match_state(bt_hw_cfg_cb_t *cfg, uint32_t mask)
{
    ...
    for (patch_entry = patch_table; patch_entry->lmp_subversion != LMP_SUBVERSION_NONE; patch_entry++)
    {
        ...
        if ((patch_entry->hci_revision_mask != HCI_REVISION_MASK_ALL) &&
            ((patch_entry->hci_revision_mask & (cfg->hci_revision > 0x1E ? cfg->hci_revision : 1 <<
                                                cfg->hci_revision)) == 0))
        {
            continue;
        }
...
**
*******************************************************************************/
static uint8_t hw_config_set_controller_baudrate(uint8_t *p_buf, uint32_t baudrate)
{
    ...
    *p++ = 4; /* parameter length */
    ...
    retval = userial_vendor_send_cmd_to_controller(p_buf, HCI_CMD_MIN_SIZE + 4, hw_config_cback);
...
static void rtk_get_bt_final_patch(bt_hw_cfg_cb_t *cfg_cb)
{
    ...
    {
        if (cfg_cb->lmp_subversion == 0x8761 && cfg_cb->hci_version == 0xc)
        {
...
__attribute__((unused)) static int hci_vendor_spec_write(uint8_t *p_buf)
{
    uint8_t retval = FALSE;
    uint8_t *p = p_buf;
    ...
    *p++ = 0x10;
    ...
    retval = userial_vendor_send_cmd_to_controller(p_buf, HCI_CMD_MIN_SIZE + 7, hw_config_cback);
    return retval;
}

static int hci_download_patch_h4(uint8_t *p_buf, int index, uint8_t *data, int len)
{
    uint8_t retval = FALSE;
    uint8_t *p = p_buf;
    ...
    *p++ = index;
    ...
    retval = userial_vendor_send_cmd_to_controller(p_buf, HCI_CMD_MIN_SIZE + len + 1, hw_config_cback);
    return retval;
}
...
//    if (p_buf != NULL)
    {
        ...
        case HW_CFG_H5_INIT:
            ...
            *p++ = 5;
            ...
            is_proceeding = userial_vendor_send_cmd_to_controller(p_buf, HCI_CMD_MIN_SIZE + 5, hw_config_cback);
            break;
        ...
        case HW_CFG_READ_FC61_HCI_SUB:
            ...
            *p = 0;
            is_proceeding = userial_vendor_send_cmd_to_controller(p_buf, HCI_CMD_MIN_SIZE, hw_config_cback);
            break;
CFG_LMP:
        ...
        case HW_CFG_READ_CHIP_TYPE:
            {
                ...
                BTVNDDBG("READ_CHIP_TYPE status = %d, length = %d", status, evt_len);
                p = p_evt_buf;
                for (i = 0; i < evt_len; i++)
                {
                ...
                if (status == 0)
                {
                    if (hw_cfg_cb.lmp_subversion == 0x8761)
                    {
                        hw_cfg_cb.chip_type = ((*(p_evt_buf +
                                                  HCI_EVT_CMD_CMPL_OPFC61_8761_CHIPTYPE_OFFSET)) & 0x0F);
                    }
                    else
                    {
                        hw_cfg_cb.chip_type = ((*(p_evt_buf + HCI_EVT_CMD_CMPL_OPFC61_CHIPTYPE_OFFSET)) &
                                               0x0F);
                    }
                    ...
                    if ((hw_cfg_cb.lmp_subversion == 0x8852) && (hw_cfg_cb.hci_revision == 0x0b))
                    {
                        ...
                        case 6:
                        case 10:
                            hw_cfg_cb.eversion = ((*(p_evt_buf + HCI_EVT_CMD_CMPL_OPFC61_CHIPDIE_OFFSET)) &
...
void hw_config_start(char transtype)
{
    ...
    if ((transtype & RTKBT_TRANS_H4) || (transtype & RTKBT_TRANS_H45))
    {
        ...
        *p = 0;
        ...
        hw_cfg_cb.state = HW_CFG_READ_LOCAL_VER;
        BTVNDDBG("hw_config_start, HCI_READ_LMP_VERSION");
        userial_vendor_send_cmd_to_controller(p_buf, HCI_CMD_MIN_SIZE, hw_config_cback);
    }
    else
    {
        UINT8_TO_STREAM(p, DATA_TYPE_COMMAND);
