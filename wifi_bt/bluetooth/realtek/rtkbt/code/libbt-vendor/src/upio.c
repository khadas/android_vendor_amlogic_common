...
int bt_wake_up_host_mode_set(uint8_t mode)
{
    ...
    int fd;
    int ret = -1;
    ALOGI("bt_wake_up_host_mode_set");

    snprintf(path, sizeof(path), "/proc/bluetooth/sleep/lpm");
    ALOGI("bt_wake_up_host_mode_set path:%s", path);
    ...
    ALOGI("bt_wake_up_host_mode_set fd:%d = open(path, O_RDWR): open(%s) success\n", \
          fd, path);
    ...
    ALOGI("bt_wake_up_host_mode_set buffer:%d", buffer);
