...
AC3FrameScanner::AC3FrameScanner(audio_format_t format)
 : FrameScanner(SPDIF_DATA_TYPE_AC3,
        ...
        sizeof(AC3FrameScanner::kSyncBytes), AC3_HEADER_LENGTH)
 , mStreamType(0)
 , mSubstreamID(0)
 , mFormat(format)
{
    mAudioBlocksPerSyncFrame = 6;
    isContainsDependentFrames = false;
...
bool AC3FrameScanner::isFrameContainSixBlockCounts()
{
    // For EAC3 we don't know if we are the end until we see a
    // frame that must be at the beginning. See isFirstInBurst().
    // BUT, we know the substream block contains 6block or not!
    return ((mDataType == SPDIF_DATA_TYPE_E_AC3) && (mSubstreamBlockCounts[0] == AC3_MAX_BLOCKS_PER_SYNC_FRAME_BLOCK));
}

bool AC3FrameScanner::isFrameContainDependentFrames()
{

    return (mDataType == SPDIF_DATA_TYPE_E_AC3) && isContainsDependentFrames;
}
...
bool AC3FrameScanner::parseHeader()
{
    ...
    if (mDataType == SPDIF_DATA_TYPE_E_AC3) {
        ...
        if (mStreamType == AC3_STREAM_TYPE_1) {
            isContainsDependentFrames = true;
        }
