...
ssize_t SPDIFEncoder::write( const void *buffer, size_t numBytes )
{
    ...
    const uint8_t *bufferOffset = (const uint8_t *)buffer;
    const uint8_t *data = (const uint8_t *)buffer;
    uint8_t *syncHeader = NULL;
    ...
    while (bytesLeft > 0) {
        ...
        // Look for beginning of next encoded frame.
            if (mFramer->scan(*data)) {
                syncHeader = (uint8_t *)data;//THIS LOCATS 5BYTES AFTER SYNC WORD
                if (mByteCursor == 0) {
                    startDataBurst();
            ...
            if (mPayloadBytesPending == 0) {
                ...
                /*SWPL-58048 below patch causes the issue, so we remove it*/
                #if 0
                //condition 1:E_AC3
                //condition 2:one frame has 6block =1536frames
                //condition 3:the frame is whole.
                if (syncHeader) {
                    size_t syncWordOffset = syncHeader - bufferOffset - (AC3_HEADER_LENGTH - 1);
                    size_t availableBytes = numBytes - syncWordOffset;
                    ALOGV("%s line %d availableBytes %d mFramer->getFrameSizeBytes() %d isFrameContainSixBlockCounts() %d",
                        __func__, __LINE__, availableBytes, mFramer->getFrameSizeBytes(), mFramer->isFrameContainSixBlockCounts());
                    if ((availableBytes == mFramer->getFrameSizeBytes())
                        && mFramer->isFrameContainSixBlockCounts()
                        && !mFramer->isFrameContainDependentFrames()) {
                        flushBurstBuffer();
                    }
                }
                #endif
