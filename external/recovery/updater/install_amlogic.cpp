...
#define SUPER_RESERVED           (4*1024)
#define SUPER_GEOMETRY          (4*1024)
...
/*
 * TODO: for Q, use use, but need find a way to enable
 * condition build in Android.bp*/
...
Value* WriteBootloaderImageFn(const char* name, State* state, const std::vector<std::unique_ptr<Expr>>& argv) {
    ...
    if ((args[0]->type == LOCAL_ARGS_VALUE_STRING || (args[0]->data.size())) == 0) {
...
Value* WriteDtbImageFn(const char* name, State* state, const std::vector<std::unique_ptr<Expr>>& argv) {
    ...
    printf("start to write dtb.img to %s...\n", DTB_DEV);
    ...
    if (args[0]->type != LOCAL_ARGS_VALUE_STRING &&
        args[0]->type != LOCAL_ARGS_VALUE_BLOB) {
        return StringValue("");
    }

    fprintf(stderr, "\nstart to write dtb.img to %s...\n", DTB_DEV);
    if (args[0]->type == LOCAL_ARGS_VALUE_BLOB) {
        fprintf(stderr, "contents type: BLOB\ncontents size: %d\n", args[0]->data.size());
...
Value* OtaZipCheck(const char* name, State* state,
    ...
    char *forUpgrade_robustOta = get_bootloader_env("forUpgrade_robustOta");
    if (forUpgrade_robustOta && (!strcmp(forUpgrade_robustOta, "true"))) {
        char *reboot_status = get_bootloader_env("reboot_status");
        if ((reboot_status != NULL) && (!strcmp(reboot_status, "reboot_init"))) {
            RecoveryPreUpdate(za);
        } else {
            return StringValue(strdup("0"));
        }
    }

    check_secure = RecoverySecureCheck(za);
    if ((check_secure == SECURE_MATCH) || (check_secure == SECURE_SKIP)) {
        printf("Secure check complete.\n\n");
    } else if ((check_secure == SECURE_ERROR) || (check_secure == SECURE_FAIL)) {
        return ErrorAbort(state, kArgsParsingFailure, "Secure check failed. %s\n\n", !check_secure ? "(Not match)" : "");
    }

#ifndef RECOVERY_DISABLE_DTB_CHECK
    printf("\n-- Dtb Check...\n");

    check_dtb = RecoveryDtbCheck(za);
    if ((check_dtb == DTB_ALLOW) || (check_dtb == DTB_CONFIG_ALLOW)) {
        printf("dtb check complete or skip.\n\n");
        if (check_secure == SECURE_OLD_NEW) {
            set_stage();
            printf("upgrade from old android to new, need upgrade by two step.\n\n");
            return StringValue(strdup("1"));
        }
    } else if (check_dtb == DTB_TWO_STEP) {
        set_stage();
        printf("dtb check not match, but can upgrade by two step.\n\n");
        return StringValue(strdup("1"));
    } else if (check_dtb == DTB_ERROR) {
...
Value* RebootRecovery(const char* name, State* state, const std::vector<std::unique_ptr<Expr>>& argv) {
    ...
    if (strstr(boot.recovery, "--update_package=")) {
        ...
        //strlcat(boot.recovery, "--retry_update\n", sizeof(boot.recovery));
    }
    ...
    return ErrorAbort(state, kArgsParsingFailure, "reboot to recovery failed!\n");
}
...
Value* Reboot(const char* name, State* state, const std::vector<std::unique_ptr<Expr>>& argv) {
    ...
    return ErrorAbort(state, kArgsParsingFailure, "reboot failed!\n");
}
...
static int get_partition_free_size(const char *path, unsigned long long package_size) {
    ...
    printf("cache free size:%llu , update_size:%llu \n", buf.f_bsize*buf.f_bavail, package_size);
...
Value* BackupUpdatePackage(const char* name, State* state,
    ...
    backup_cache_flag = get_partition_free_size("/cache", state->updater->GetMappedPackageLength());
    ...
    if (path[0] == '@') {
        ...
        if (backup_cache_flag == 0) {
            ...
            printf("%s %s %d", path, offset.c_str(), state->updater->GetMappedPackageLength());
            sprintf(buf, "%s %s %d", path, offset.c_str(), state->updater->GetMappedPackageLength());
            ...
            size_t len_w = write(fd, state->updater->GetMappedPackageAddress(), state->updater->GetMappedPackageLength());
            if (len_w != state->updater->GetMappedPackageLength()) {
            ...
            int len_w = write(fd, state->updater->GetMappedPackageAddress(), state->updater->GetMappedPackageLength());
            if (len_w != state->updater->GetMappedPackageLength()) {
                printf("write %s failed!\n", UPDATE_TMP_FILE);
                close(fd);
...
Value* WriteFilePartition(const char* name, State* state, const std::vector<std::unique_ptr<Expr>>& argv) {
    ...
    if (access(file.c_str(), R_OK)) {
        printf("file %s is not exist!", file.c_str());
        return StringValue("OK");
    }

    FILE *pf = fopen(file.c_str(), "rb");
    if (pf == NULL) {
        return ErrorAbort(state, kFileOpenFailure, "read %s  failed!\n",  file.c_str());
    }

    int fd = open(partition.c_str(), O_WRONLY);
    if (fd < 0) {
        fclose(pf);
        return ErrorAbort(state, kFileOpenFailure, "read %s  failed!\n",  partition.c_str());
    }
    ...
    fseek(pf, 0, SEEK_END);
    int size = ftell(pf);
    printf("start to write %s to %s, size(%d)\n", file.c_str(), partition.c_str(), size);
    ...
    fseek(pf, 0, SEEK_SET);
    int len = fread(buf, 1, size, pf);
    if (len != size) {
        fclose(pf);
        close(fd);
        free(buf);
        return ErrorAbort(state, kFreadFailure, "read %s  failed!\n",  file.c_str());
    }
    fclose(pf);
    ...
    if (len != size) {
        close(fd);
        free(buf);
        return ErrorAbort(state, kFwriteFailure, "read %s  failed!\n",  partition.c_str());
    }
...
Value* RecoveryBackupExist(const char* name, State* state, const std::vector<std::unique_ptr<Expr>>& argv) {
    ...
    if (argv.size() != 0) {
      return ErrorAbort(state, kArgsParsingFailure, "%s() expects 0 arg, got %zu", name, argv.size());
    }
    ...
    if (FindEntry(za, RECOVERY_IMG, &entry) != 0) {
...
Value* WriteIdAttestation(const char* name, State* state, const std::vector<std::unique_ptr<Expr>>& argv) {
    int iRet = 0;
    ...
    std::string serial_num = android::base::GetProperty("ro.boot.serialno", "");
    ...
    std::string manufacturer_name = android::base::GetProperty("ro.product.manufacturer", "");
    std::string model_name = android::base::GetProperty("ro.product.model", "");

    XMLDocument doc;
    doc.Parse(args[0]->data.c_str());

    XMLElement* titleElement = doc.FirstChildElement("ATTESTATION_ID")->
        FirstChildElement("SERIAL")->FirstChildElement("val");
    const char* title = titleElement->GetText();
    titleElement->SetText(serial_num.c_str());

    titleElement = doc.FirstChildElement("ATTESTATION_ID")->
        FirstChildElement("BRAND")->FirstChildElement("val");
    title = titleElement->GetText();
    titleElement->SetText(brand_name.c_str());

    titleElement = doc.FirstChildElement("ATTESTATION_ID")->
        FirstChildElement("DEVICE")->FirstChildElement("val");
    title = titleElement->GetText();
    titleElement->SetText(device_name.c_str());

    titleElement = doc.FirstChildElement("ATTESTATION_ID")->
        FirstChildElement("PRODUCT")->FirstChildElement("val");
    title = titleElement->GetText();
    titleElement->SetText(product_name.c_str());

    titleElement = doc.FirstChildElement("ATTESTATION_ID")->
        FirstChildElement("MANUFACTURER")->FirstChildElement("val");
    title = titleElement->GetText();
    titleElement->SetText(manufacturer_name.c_str());

    titleElement = doc.FirstChildElement("ATTESTATION_ID")->
        FirstChildElement("MODEL")->FirstChildElement("val");
    title = titleElement->GetText();
    titleElement->SetText(model_name.c_str());

    doc.SaveFile("/tmp/id_attestation.xml");

    int fd = 0;
    int len = 0;
    char buf[1024];
    fd = open("/tmp/id_attestation.xml", O_RDONLY);
    if (fd < 0) {
        printf("open /tmp/id_attestation.xml failed!\n");
        return StringValue(strdup("0"));
    }
    ...
    iRet = set_attestationkey(buf);
    if (iRet == 0) {
        printf("Write attestation id successful!\n\n");
    } else {
        printf("Write attestation id failed!\n\n");
        close(fd);
        return StringValue(strdup("1"));
    }
    close(fd);
...
Value* WriteSuperEmpty(const char* name, State* state, const std::vector<std::unique_ptr<Expr>>& argv) {
    ...
    ret = lseek(fd, SUPER_RESERVED, SEEK_SET);
    if (ret != SUPER_RESERVED) {
        ErrorAbort(state, kLseekFailure, "lseek super partition failed\n");
        return nullptr;
    }
    ret = write_data(fd, args[0]->data.c_str(), SUPER_GEOMETRY);
    if (ret == -1) {
        ErrorAbort(state, kFwriteFailure, "write super partition failed\n");
        return nullptr;
    }
    ...
    ret = write_data(fd, args[0]->data.c_str()+SUPER_GEOMETRY, args[0]->data.size()-SUPER_GEOMETRY);
    if (ret == -1) {
        ErrorAbort(state, kFwriteFailure, "write super partition failed\n");
        return nullptr;
    }
...
void Register_libinstall_amlogic() {
    ...
    RegisterFunction("write_file_partition", WriteFilePartition);
    RegisterFunction("write_super_empty", WriteSuperEmpty);
    RegisterFunction("write_id_attestation", WriteIdAttestation);
