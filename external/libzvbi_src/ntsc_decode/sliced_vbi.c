...
read_loop_old_sliced		(struct stream *	st)
{
	...
	for (;;) {
		...
		if (st->raw_valid && st->decode_raw) {
			printf("st->raw_valid && st->decode_raw is true\n");
...
struct stream *
read_stream_new			(const char *		fbuffer,
				 int length,
				 enum file_format	file_format,
				 unsigned int		ts_pid,
				 stream_callback_fn *	callback ,
