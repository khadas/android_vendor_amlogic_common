...
import com.android.settingslib.wifi.AccessPoint;  //DroidLogic modify
...
public class OptionsOrConnectState implements State {
    ...
    public void processForward() {
        ...
        int wifiSecurity = userChoiceInfo.getWifiSecurity();
        if (status == UserChoiceInfo.ConnectionFailedStatus.AUTHENTICATION) {
            userChoiceInfo.setConnectionFailedStatus(null);
            if (AccessPoint.SECURITY_EAP == wifiSecurity) {
                // DroidLogic start
                stateMachine.getListener().onComplete(StateMachine.IDENTITY);
                // DroidLogic end
            } else {
