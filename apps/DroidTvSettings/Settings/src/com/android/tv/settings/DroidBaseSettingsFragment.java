...
import com.android.tv.settings.system.DroidLeanbackPickerDialogFragment;
...
public abstract class DroidBaseSettingsFragment extends LeanbackSettingsFragment {
    @Override
    ...
    public boolean onPreferenceDisplayDialog(@NonNull PreferenceFragment caller, Preference pref) {
        ...
        if (pref instanceof LeanbackPickerDialogPreference) {
            ...
            f = dialogPreference.getType().equals("date")
                    ? DroidLeanbackPickerDialogFragment.newDatePickerInstance(pref.getKey())
                    : DroidLeanbackPickerDialogFragment.newTimePickerInstance(pref.getKey());
            f.setTargetFragment(caller, 0);
            startPreferenceFragment(f);
            return true;
        } else {
