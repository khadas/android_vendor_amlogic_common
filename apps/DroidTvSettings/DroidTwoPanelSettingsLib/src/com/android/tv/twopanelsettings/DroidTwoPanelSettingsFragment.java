...
import com.android.tv.twopanelsettings.slices.DroidInfoFragment;
 ...
 * information.
 */
public abstract class DroidTwoPanelSettingsFragment extends Fragment implements
        PreferenceFragment.OnPreferenceStartFragmentCallback,
        PreferenceFragment.OnPreferenceStartScreenCallback,
        PreferenceFragment.OnPreferenceDisplayDialogCallback {
    private static final String TAG = "DroidTwoPanelSettingsFragment";
    private static final boolean DEBUG = false;
    private static final String PREVIEW_FRAGMENT_TAG =
            "com.android.tv.settings.DroidTwoPanelSettingsFragment.PREVIEW_FRAGMENT";
    private static final String PREFERENCE_FRAGMENT_TAG =
            "com.android.tv.settings.DroidTwoPanelSettingsFragment.PREFERENCE_FRAGMENT";
    ...
    private boolean shouldDisplay(String fragment) {
        try {
            return LeanbackPreferenceFragment.class.isAssignableFrom(Class.forName(fragment))
                    || DroidInfoFragment.class.isAssignableFrom(Class.forName(fragment));
    ...
    public boolean onPreferenceStartFragment(PreferenceFragment caller, Preference pref) {
        ...
        if (preview != null && !(preview instanceof DummyFragment)) {
            if (!(preview instanceof DroidInfoFragment)) {
        ...
        public boolean onKey(View v, int keyCode, KeyEvent event) {
            ...
            if (event.getAction() == KeyEvent.ACTION_DOWN
                ...
                if (shouldPerformClick()) {
                    ...
                    if (!(previewFragment instanceof DroidInfoFragment)) {
                        navigateToPreviewFragment();
                    }
                }
    ...
    private void moveToPanel(final int index, boolean smoothScroll) {
        mHandler.post(() -> {
            ...
            boolean setAlphaForPreview = fragmentToBecomePreviewPanel != null
                    ...
                    && !(fragmentToBecomePreviewPanel instanceof DroidInfoFragment);
    ...
    /**
     * Refocus the current selected preference. When a preference is selected and its DroidInfoFragment
     * slice data changes. We need to call this method to make sure DroidInfoFragment updates in time.
        ...
        try {
            if (chosenPreference != null) {
                if (chosenPreference.getFragment() != null
                        && DroidInfoFragment.class.isAssignableFrom(
    ...
    public void showErrorMessage(String errorMessage) {
        Fragment prefFragment =
                getChildFragmentManager().findFragmentById(frameResIds[mPrefPanelIdx]);
        Preference preference = getChosenPreference(prefFragment);
        preference.setFragment(DroidInfoFragment.class.getCanonicalName());
    ...
    private void updateInfoFragmentStatus(Fragment fragment) {
        ...
        final Fragment existingPreviewFragment =
                getChildFragmentManager().findFragmentById(frameResIds[mPrefPanelIdx + 1]);
        if (existingPreviewFragment instanceof DroidInfoFragment) {
            ((DroidInfoFragment) existingPreviewFragment).updateInfoFragment();
        }
    }
    ...
    public Fragment onCreatePreviewFragment(Fragment caller, Preference preference) {
        if (preference.getFragment() != null) {
            ...
            if (preference instanceof ListPreference) {
                f = DroidTwoPanelListPreferenceDialogFragment.newInstanceSingle(preference.getKey());
