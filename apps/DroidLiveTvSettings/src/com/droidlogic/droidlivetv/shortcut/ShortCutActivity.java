...
public class ShortCutActivity extends Activity implements ListItemSelectedListener, OnItemClickListener {
    ...
    private void setGuideView() {
        ...
        tx_program_genre.setVisibility(View.GONE);
        tx_program_rating.setVisibility(View.GONE);
    ...
    private void loadDateList() {
        ...
        list_date = getDateList(saveChannelIndex);
    ...
    private ArrayList<ArrayMap<String, Object>> getDateList(int saveChannelIndex) {
        ArrayList<ArrayMap<String, Object>> dates = new ArrayList<ArrayMap<String, Object>>();

        if (saveChannelIndex < 0 || channelInfoList.size() <= saveChannelIndex) {
            Log.d(TAG, "getDateList invalid saveChannelIndex=" + saveChannelIndex + " list size=" + channelInfoList.size());
            return getNoProgramDateList(dates, saveChannelIndex);
        }
        ...
        if (channel_programs.isEmpty()) {
            Log.e(TAG, "getDateList no programs for channel " + channel_programs);
            return getNoProgramDateList(dates, saveChannelIndex);
        }
        ...
        while ((tmp_time <= (lastProgramTime + time_offset)) && count < 10) {//show 10 days at most
            ...
            ArrayMap<String, Object> item = new ArrayMap<String, Object>();
            //get tz time date, to fix h/m/s with 00:00:00, we use utc to avoid adjust with tz
            String[] dateAndTime = getDateAndTime(tmp_time, true);
            Log.d(TAG, "current tmp time: " + Arrays.toString(dateAndTime));
            item.put(GuideListView.ITEM_1, dateAndTime[1] + "." + dateAndTime[2]);
            item.put(GuideListView.ITEM_2, Long.toString(tmp_time - time_offset));
            tmp_time = tmp_time + DAY_TO_MS;
            item.put(GuideListView.ITEM_3, Long.toString(tmp_time - time_offset - 1));
            if (saveChannelIndex != currentChannelIndex) {
                Log.e(TAG, "getDateList! save=" + saveChannelIndex + " current=" + currentChannelIndex);
                return dates;
            }

            /*ignore the days before today*/
            if (tmp_time <= (currentStreamTime + time_offset)) {
                continue;
            }
            count++;
            dates.add(item);
    ...
    private ArrayList<ArrayMap<String, Object>> getNoProgramDateList(
        ...
        if (saveChannelIndex != currentChannelIndex) {
            Log.e(TAG, "getNoProgramDateList! save=" + saveChannelIndex + " current=" + currentChannelIndex);
            return dates;
        }
        dates.add(item);
        return dates;
            ...
            case R.id.list_guide_programs:
                ...
                if (position < list_program.size()) {
                    ...
                    if (TextUtils.isEmpty(jsonObjStr)) {
                        ...
                        tx_program_rating.setVisibility(View.GONE);
                        tx_program_extend.setVisibility(View.GONE);
                        tx_program_description.setVisibility(View.VISIBLE);
                        tx_program_description.setText(getString(R.string.no_information));
                    } else {
                        JSONObject jsonObj = new JSONObject();
                        try {
                            jsonObj = new JSONObject(jsonObjStr.toString());
                            program_event = jsonObj.getString("program_event");
                        } catch (Exception e) {
                            Log.e(TAG, "onListItemSelected program_event Exception = " + e.getMessage());
                        }
                        try {
                            program_genre = jsonObj.getString("program_genre");
                        } catch (Exception e) {
                            Log.e(TAG, "onListItemSelected program_genre Exception = " + e.getMessage());
                        }
                        try {
                            program_rating = jsonObj.getString("program_rating");
                        } catch (JSONException e) {
                            Log.e(TAG, "onListItemSelected program_rating Exception = " + e.getMessage());
                        }
                        try {
                            program_short = jsonObj.getString("program_short");
                        } catch (JSONException e) {
                            Log.e(TAG, "onListItemSelected program_short Exception = " + e.getMessage());
                        }
                        try {
                            program_extend = jsonObj.getString("program_extend");
                        } catch (JSONException e) {
                            Log.e(TAG, "onListItemSelected program_extend Exception = " + e.getMessage());
                        }
                        if (TextUtils.isEmpty(program_event)) {
                            if (tx_program_event_name.getVisibility() != View.GONE) {
                                tx_program_event_name.setVisibility(View.GONE);
                            }
                        } else {
                            tx_program_event_name.setText(getString(R.string.program_event_name) + "\n" + program_event);
                            if (tx_program_event_name.getVisibility() != View.VISIBLE) {
                                tx_program_event_name.setVisibility(View.VISIBLE);
                            }
                        }
                        if (TextUtils.isEmpty(program_genre)) {
                            if (tx_program_genre.getVisibility() != View.GONE) {
                                tx_program_genre.setVisibility(View.GONE);
                            }
                        } else {
                            tx_program_genre.setText(getString(R.string.program_genre) + "\n" + program_genre);
                            if (tx_program_genre.getVisibility() != View.VISIBLE) {
                                tx_program_genre.setVisibility(View.VISIBLE);
                            }
                        }
                        if (TextUtils.isEmpty(program_rating)) {
                            if (tx_program_rating.getVisibility() != View.GONE) {
                                tx_program_rating.setVisibility(View.GONE);
                            }
                        } else {
                            tx_program_rating.setText(getString(R.string.program_rating) + "\n" + program_rating);
                            if (tx_program_rating.getVisibility() != View.VISIBLE) {
                                tx_program_rating.setVisibility(View.VISIBLE);
                            }
                        }
                        if (TextUtils.isEmpty(program_short)) {
                            if (tx_program_description.getVisibility() != View.GONE) {
                                tx_program_description.setVisibility(View.GONE);
                            }
                        } else {
                            tx_program_description.setText(getString(R.string.program_short) + "\n" + program_short);
                            if (tx_program_description.getVisibility() != View.VISIBLE) {
                                tx_program_description.setVisibility(View.VISIBLE);
                            }
                        }
                        if (TextUtils.isEmpty(program_extend)) {
                            if (tx_program_extend.getVisibility() != View.GONE) {
                                tx_program_extend.setVisibility(View.GONE);
                            }
                        } else {
                            tx_program_extend.setText(getString(R.string.program_extend) + "\n" + program_extend);
                            if (tx_program_extend.getVisibility() != View.VISIBLE) {
                                tx_program_extend.setVisibility(View.VISIBLE);
                            }
                        }
                    }
                }
    ...
    public String[] getDateAndTime(long dateTime) {
        return getDateAndTime(dateTime, false);
    }

    public String[] getDateAndTime(long dateTime, boolean isUTC) {
        SimpleDateFormat sDateFormat = new SimpleDateFormat("yyyy/MM/dd HH:mm:ss");
        if (isUTC) {
    ...
    private final class ProgramObserver extends ContentObserver {
        ...
        public void onChange(boolean selfChange, Uri uri) {
            ...
            if (program != null &&
                ...
                Log.d(TAG, "current channel update " + currentChannel);
                mThreadHandler.removeMessages(MSG_LOAD_DATE);
                mThreadHandler.removeMessages(MSG_LOAD_PROGRAM);
