...
`com.droidlogic.android.abupdater.UpdateConfig`, droidlogic file is located
...
`/data/user/0/com.droidlogic.android.abupdater/files/configs/`.

ABUpdater app downloads OTA package from `url`. In this sample app
...
- `Updater state:` - ABUpdater app state.
...
2. Add the module (ABUpdater) to the `PRODUCT_PACKAGES` list for the
   lunch target.
...
1. Build `make -j ABUpdater` and `make -j ABUpdaterTests`.
2. Install app
   `adb install $OUT/system/app/ABUpdater/ABUpdater.apk`
3. Install tests
   `adb install $OUT/testcases/ABUpdaterTests/arm64/ABUpdaterTests.apk`
4. Run tests
   `adb shell am instrument -w com.droidlogic.android.abupdater.tests/android.support.test.runner.AndroidJUnitRunner`
5. Run a test file
   ```
   adb shell am instrument \
     -w -e class com.droidlogic.android.abupdater.UpdateManagerTest#applyUpdate_appliesPayloadToUpdateEngine \
...
ABUpdater app is released under
