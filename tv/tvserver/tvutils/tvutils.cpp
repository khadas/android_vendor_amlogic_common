...
vpp_picture_mode_t tvGetLastPQMode ( void )
{
    const sp<SystemControlClient> &sws = getSystemControlService();
    if (sws != nullptr) {
        return VPP_PICTURE_MODE_MAX;//(vpp_picture_mode_t)sws->getLastPQmode();
    }
...
int64_t jsonGetInt64(const char *json, const char *obj, const char *value, int64_t def)
{
    ...
    if (obj && strlen(obj))
        return root.get(obj, Json::Value()).get(value, def).asInt64();
    return root.get(value, def).asInt64();
}
...
int getKernelMajorVersion(void)
{
    int retVal = 0;
    char buf[16] = {0};
    int readRet = readSys("/proc/version", buf, sizeof(buf));
    if (readRet < 0) {
        retVal = 0;
    } else {
        retVal = (int)(buf[14] - '0');
