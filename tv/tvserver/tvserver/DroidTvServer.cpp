...
Return<int32_t> DroidTvServer::processCmd(int32_t type __unused, int32_t size __unused) {
    #if 0
    Parcel p;
...
Return<int32_t> DroidTvServer::loadEdidData(int isNeedBlackScreen, int isDolbyVisionEnable) {
    return mTvServiceIntf->loadEdidData(isNeedBlackScreen, isDolbyVisionEnable);
}

Return<int32_t> DroidTvServer::updateEdidData(int32_t inputSrc, const hidl_string& edidData) {
    return mTvServiceIntf->updateEdidData(inputSrc, edidData);
}
...
Return<void> DroidTvServer::getHdmiFormatInfo(getHdmiFormatInfo_cb _hidl_cb) {
    FormatInfo info;
    mTvServiceIntf->getHdmiFormatInfo(info.width, info.height, info.fps);
    info.interlace = 0;
    _hidl_cb(info);
    return Void();
}
...
Return<int32_t> DroidTvServer::setScreenColorForSignalChange(int32_t screenColor, int32_t is_save) {
    return mTvServiceIntf->setScreenColorForSignalChange(screenColor, is_save);
}

Return<int32_t> DroidTvServer::getScreenColorForSignalChange() {
    return mTvServiceIntf->getScreenColorForSignalChange();
}
...
Return<int32_t> DroidTvServer::dtvGetSignalSNR() {
    return mTvServiceIntf->dtvGetSignalSNR();
}
...
Return<void> DroidTvServer::request(const hidl_string& resource, const hidl_string& paras, request_cb _hidl_cb) {
    std::string ret = mTvServiceIntf->request(resource, paras);

    LOGI("%s, cmd:%s params:%s, ret:%s", __FUNCTION__, resource.c_str(), paras.c_str(), ret.c_str());
    _hidl_cb(ret);
    return Void();
}
