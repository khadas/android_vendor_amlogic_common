...
void DroidTvServiceIntf::onTvEvent(const CTvEv &ev)
{
    ...
    case CTvEv::TV_EVENT_PLAY_INSTANCE: {
        TvHidlParcel hidlParcel;
        hidlParcel.msgType = TSPLAYER_INSTANCENO_CALLBACK;

        TvEvent::PlayerInstanceEvent *pEv = (TvEvent::PlayerInstanceEvent *)(&ev);

        hidlParcel.bodyInt.resize(3);
        hidlParcel.bodyInt[0] = pEv->mMsgType;
        hidlParcel.bodyInt[1] = pEv->type;
        hidlParcel.bodyInt[2] = pEv->id;
    ...
    case CTvEv::TV_EVENT_CHECK_SOURCE_VALID: {
        TvEvent::CheckSourceValidEvent *pEv = (TvEvent::CheckSourceValidEvent *)(&ev);

        TvHidlParcel hidlParcel;
        hidlParcel.msgType = CHECK_SOURCE_VALID;
        hidlParcel.bodyInt.resize(2);
        hidlParcel.bodyInt[0] = pEv->cmd;
        hidlParcel.bodyInt[1] = pEv->source;
        mNotifyListener->onEvent(hidlParcel);
        break;
    }
...
std::string DroidTvServiceIntf::getSupportInputDevices() {
    ...
    std::string ret =  std::string(valueStr);
    LOGD("get support input devices = %s", valueStr);

    // sometime need 1 image compatible with atsc or dtvkit platforms
    //then we use "force_dtvkit_source" env to check witch source we need start
    char forceDTVKitSource[128] = {'\0'};
    if (getBootEnv("ubootenv.var.force_dtvkit_source", forceDTVKitSource, "false") == 0) {
        if (strcmp("true", forceDTVKitSource) == 0) {
            LOGD("forceDTVKitSource = true");
            std::string::size_type pos(0);
            std::string adtv = std::string(",16");
            std::string dtvkit = std::string(",19");
            if ((pos = ret.find(adtv,pos)) == std::string::npos) {
                // ret.replace(pos, adtv.length(), dtvkit);
                ret += adtv;
            }

            pos = 0;
            if ((pos = ret.find(dtvkit,pos)) == std::string::npos) {
                ret += dtvkit;
            }
        }
    }
...
int DroidTvServiceIntf::loadEdidData(int32_t isNeedBlackScreen, int32_t isDolbyVisionEnable) {
    return mpTv->LoadEdidData(isNeedBlackScreen, isDolbyVisionEnable);
}

int DroidTvServiceIntf::updateEdidData(int32_t inputSrc, const std::string& edidData) {
    return mpTv->UpdateEdidData((tv_source_input_t)inputSrc, (unsigned char *)edidData.c_str());
}
...
int DroidTvServiceIntf::getHdmiFormatInfo(int &srcWidth, int &srcHeight, int &srcFps)
{
    return mpTv->getHdmiFormatInfo(&srcWidth, &srcHeight, &srcFps);
}
...
int DroidTvServiceIntf::setBlackoutEnable(int32_t status, int32_t is_save) {
    //function has been moved to systemcontrol
    return -1;
}
...
int DroidTvServiceIntf::setScreenColorForSignalChange(int32_t screenColor, int32_t is_save) {
    //function has been moved to systemcontrol
    return -1;
}

int DroidTvServiceIntf::getScreenColorForSignalChange() {
    //function has been moved to systemcontrol
    return -1;
}
...
int DroidTvServiceIntf::setAmAudioPreMute(int32_t mute __unused) {
...
int DroidTvServiceIntf::dtvGetSignalSNR() {
    return mpTv->getFrontendSNR();
}
...
void DroidTvServiceIntf::dtvGetScanFreqListMode(int mode, std::vector<FreqList> &freqlist) {
    ...
    CTvRegion::getChannelListByName((char *)CTvScanner::getDtvScanListName(mode), out);
...
int DroidTvServiceIntf::updateEAS(int freq __unused, int moudle __unused, int mode __unused) {
...
std::string DroidTvServiceIntf::request(const std::string& resource, const std::string& paras) {
    return mpTv->request(resource, paras);
}
...
status_t DroidTvServiceIntf::dump(int fd __unused, const Vector<String16>& args __unused)
{
#if 0
    String8 result;
