...
int CFrontEnd::Open(int mode)
{
    ...
    if (mbFEOpened == false) {
        ...
        /* when open with different mode, need set mode. */
        if (mCurMode != mode && TV_FE_ANALOG != mode && mode != FE_AUTO) {
            LOGD("FE Open [%d->%d]", mCurMode, mode);
...
int CFrontEnd::setPara(const char *paras, bool force )
{
    ...
    case TV_FE_OFDM:
        ...
        dvbfepara.terrestrial.ofdm_mode = (int)mFEParas.getFEMode().getGen();
        break;
    case TV_FE_DTMB:
...
int CFrontEnd::SNR10ToSQI(int snr)
{
    int sqi = 0;

    sqi = snr / 10;

    LOGD("%s, snr %d, sqi %d\n", __FUNCTION__, snr, sqi);

    return sqi;
}

int CFrontEnd::StrengthToSSI(int strength)
{
    int ssi = 0;
    ...
    LOGD("%s, strength %d, ssi %d\n", __FUNCTION__, strength, ssi);
...
#ifdef SUPPORT_ADTV
    return 0;/*no need*/
    //this->setPara ( TV_FE_ANALOG, 44250000, V4L2_COLOR_STD_PAL | V4L2_STD_PAL_DK, 0 );
#else
    return -1;
#endif
}
...
{
    LOGD("%s: value is %d", __FUNCTION__, onOff);
    FILE *fp = NULL;
    int ret = -1;

    fp = fopen(PATH_FE_TIMER_EN_49, "w");

    if ( fp == NULL ) {
        LOGW ("Open %s error(%s)!\n", PATH_FE_TIMER_EN_49, strerror(errno));
        return -1;
    }
    if (onOff)
        ret = fprintf(fp, "%s", "1");
...
int CFrontEnd::SetAnalogFrontEndSearhSlowMode(int onOff)
{
    ...
    fp = fopen(PATH_FE_SLOW_MODE_49, "w");

    if ( fp == NULL ) {
        LOGW ( "Open %s error(%s)!\n", PATH_FE_SLOW_MODE_49, strerror ( errno ) );
        return -1;
    }
    if (onOff)
        ret = fprintf(fp, "%s", "1");
