...
int CTvScanner::Scan(CFrontEnd::FEParas &fp, ScanParas &sp) {
    ...
    para.fend_dev_id = config_get_int(CFG_SECTION_TV, FRONTEND_DTV_DEVICE, CFrontEnd::FE_DEV_ID);
    para.vlfend_dev_id = CFrontEnd::VLFE_DEV_ID;
    ...
    //default should skip services only in vct while not in pmt
    mode = config_get_int(CFG_SECTION_TV, CFG_DTV_SCAN_STOREMODE_NOVCT, 1);
    if (mode == 1) {
    ...
    for (i = 0; i < size; i++) {
        ...
        switch (dtv_para.fe_paras[i].m_type) {
            ...
            case TV_FE_OFDM:
                ...
                dtv_para.fe_paras[i].terrestrial.ofdm_mode = (int)fp.getFEMode().getGen();
                if (fp.getBandwidth() != -1)
...
const char *CTvScanner::getDtvScanListName(int mode)
{
    ...
    switch (base) {
        ...
        case TV_FE_ISDBT:
            if (strcmp(pCountry, "PH") == 0) {
                list_name = (char *)"PH,Default ISDBT";
            } else if (strcmp(pCountry, "BR") == 0) {
