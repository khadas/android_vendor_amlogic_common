...
int SSMSaveScreenColorForSignalChange(int screen_color)
{
    return TVSSMWriteNTypes(CUSTOMER_DATA_POS_SCREEN_COLOR_START, 1, screen_color);
}

int SSMReadScreenColorForSignalChange(int *screen_color)
{
    int tmp_val = 0;
    int ret = TVSSMReadNTypes(CUSTOMER_DATA_POS_SCREEN_COLOR_START, 1, &tmp_val);
    *screen_color = tmp_val;
...
tv_hdmi_edid_version_t SSMReadHDMIEdidVersion(tv_hdmi_port_id_t port)
{
    ...
    if ((tmp_ret < HDMI_EDID_VER_14) || (tmp_val >= HDMI_EDID_VER_MAX)) {
        LOGD("%s failed!\n", __FUNCTION__);
        tmp_val = 0;
    }
