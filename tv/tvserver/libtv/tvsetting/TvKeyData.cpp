...
int RealHandleHDCPKey(unsigned char hdcp_key_buf[])
{
    ...
    dev_fd = open(CS_KEY_EDID_HDCP_DEV_PATH, O_RDWR);
    if (dev_fd < 0) {
...
{
    return RealHandleHDMIEdid(customer_hdmi_edid_buf, CC_CUSTOMER_HDMI_EDID_TOTAL_SIZE);
}
...
int RealHandleHDMIEdid(unsigned char hdmi_edid_buf[], int write_size)
{
    ...
    if (write(dev_fd, hdmi_edid_buf, write_size)
            < 0) {
        close(dev_fd);
        dev_fd = -1;
        LOGE("%s, write edid file ERROR(%s)!!\n", CFG_SECTION_TV, strerror(errno));
...
static int ReadDataFromFile(char *file_name, int offset, int nsize, unsigned char data_buf[])
{
    int device_fd = -1;
    int tmp_ret = 0;
    char *tmp_ptr = NULL;
...
bool IsNeedLoadDolbyVisionEdid(int isDolbyVisionEnable)
{
    ...
    if (isDolbyVisionKoExist && (isDolbyVisionEnable == 1)) {
        isLoadDolbyEdid = true;
    } else {
        isLoadDolbyEdid = false;
    }

    return isLoadDolbyEdid;
}

int SSMLoadHDMIEdidData(bool isLoadDolbyEdid)
{
    int ret = 0;
    unsigned char hdmi_edid_data_load_buf[CC_CUSTOMER_HDMI_EDID_TOTAL_SIZE_NEW];
    unsigned char hdmi_edid_data_read_buf[SSM_HDMI_EDID_SIZE];
            ...
            if (isLoadDolbyEdid) {
               LOGD("%s: load dolby vision EDID!\n", __FUNCTION__);
                if (loadNum%2 != 0) {
                    ...
                    sprintf(edidFilePath, "%s/port%d_20_dv.bin", edidDirPath, loadNum/2);
                }
            } else {
                LOGD("%s: load ordinary EDID!\n", __FUNCTION__);
                if (loadNum%2 != 0) {
                    sprintf(edidFilePath, "%s/port%d_14.bin", edidDirPath, loadNum/2 + 1);
                } else {
                    sprintf(edidFilePath, "%s/port%d_20.bin", edidDirPath, loadNum/2);
                }
            }
            memset(hdmi_edid_data_read_buf, 0, SSM_HDMI_EDID_SIZE);
            ReadDataFromFile(edidFilePath, 0, SSM_HDMI_EDID_SIZE, hdmi_edid_data_read_buf);
            memcpy(hdmi_edid_data_load_buf + (loadNum-1)*SSM_HDMI_EDID_SIZE, hdmi_edid_data_read_buf, SSM_HDMI_EDID_SIZE);
        }
        ret = RealHandleHDMIEdid(hdmi_edid_data_load_buf, SSM_HDMI_EDID_SIZE * 6);
    }else {
        //EDID14
        memset(hdmi_edid_data_read_buf, 0, SSM_HDMI_EDID_SIZE);
        ReadDataFromFile((char *)TV_CONFIG_EDID14_FILE_PATH, 0, SSM_HDMI_EDID_SIZE, hdmi_edid_data_read_buf);
        memcpy(hdmi_edid_data_load_buf, hdmi_edid_data_read_buf, SSM_HDMI_EDID_SIZE);
        //EDID20
        memset(hdmi_edid_data_read_buf, 0, SSM_HDMI_EDID_SIZE);
        ReadDataFromFile((char *)TV_CONFIG_EDID20_FILE_PATH, 0, SSM_HDMI_EDID_SIZE, hdmi_edid_data_read_buf);
        memcpy(hdmi_edid_data_load_buf+SSM_HDMI_EDID_SIZE, hdmi_edid_data_read_buf, SSM_HDMI_EDID_SIZE);

        ret = RealHandleHDMIEdid(hdmi_edid_data_load_buf, SSM_HDMI_EDID_SIZE * 2);
    }
...
int SSMSetHDMIEdidVersion(char *value)
{
    ...
    int dev_fd = open(CS_HDMI_EDID_VERSION_DEV_PATH, O_RDWR);
    if (dev_fd < 0) {
        LOGE("%s: open %s ERROR(%s)!!\n", __FUNCTION__, CS_HDMI_EDID_VERSION_DEV_PATH, strerror(errno));
        return -1;
    }

    if (write(dev_fd, value, strlen(value)) < 0) {
        close(dev_fd);
        dev_fd = -1;
