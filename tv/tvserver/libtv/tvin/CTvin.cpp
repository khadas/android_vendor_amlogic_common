...
CTvin::CTvin() : mAfeDevFd(-1), mVdin0DevFd(-1)
{
    ...
    mResmanFd = -1;
    mOnPreemtRegisted = false;
    mSupportResman = resman_support();
...
int CTvin::setMpeg2Vdin(int enable)
{
    ...
    if (kernelVersion > 4) {
        return tvWriteSysfs(PATH_DI_MPEG2VDIN_EN_54, str);
    } else {
        return tvWriteSysfs(PATH_DI_MPEG2VDIN_EN_49, str);
    }
}
...
{
    int ret = -1;
    if (mSupportResman && !Resman_Request(RESMAN_APP_XDVB, RESMAN_ID_VFM_DEFAULT, 2000, NULL)) {
        return ret;
    }
    ...
    if (mSupportResman) {
        Resman_FreeRes(RESMAN_ID_VFM_DEFAULT);
    }
...
int CTvin::VDIN_AddVideoPath ( int selPath )
{
    ...
    if (bNeedAcqVfm) {
        if (!Resman_Request(RESMAN_APP_XDVB, RESMAN_ID_VFM_DEFAULT, 2000, NULL)) {
            return -1;
        }
    }

    ret = tvReadSysfs("/sys/class/vdetect/tv_add_vdetect", buf);
    add_vdetect = atoi(buf);
    if (add_vdetect) {
        vdinPath += videodetectPath;
    }

    if (fixed_tunnel && (selPath == TV_PATH_VDIN_AMLVIDEO2_PPMGR_DEINTERLACE_AMVIDEO)) {
        vdinDvPath = "add dvhdmiin dv_vdin videoqueue.0";
        ret = VDIN_RmHdmiDvPath();
        if (ret > 0) {
            LOGD("remove hdmiddv path ok\n");
            ret = VDIN_AddPath(vdinDvPath.c_str());
            if (ret > 0)
                LOGD("add hdmiddv path ok\n");
            else
                LOGW("add hdmiddv path faild\n");
        } else {
...
int CTvin::VDIN_OpenPort ( tvin_port_t port )
{
    ...
    m_tvin_param.port = port;

    if (!rt && mSupportResman && TVIN_PORT_CVBS0 <= port && port <= TVIN_PORT_CVBS7) {
        if (!Resman_Request(RESMAN_APP_XDVB, RESMAN_ID_ADC_PLL, 2000, NULL)) {
            LOGD ( "%s, Resman_Request RESMAN_ID_ADC_PLL failed, source_port = %x",
                __FUNCTION__,  port);
            return -1;
        }
        LOGD ( "%s, Resman_Request RESMAN_ID_ADC_PLL OK, source_port = %x",
                __FUNCTION__,  port);
    }
...
int CTvin::VDIN_ClosePort()
{
    ...
    if (mSupportResman && TVIN_PORT_CVBS0 <= m_tvin_param.port && m_tvin_param.port <= TVIN_PORT_CVBS7) {
        ...
        LOGD ( "%s, Resman_Free RESMAN_ID_ADC_PLL OK, source_port = %x",
                __FUNCTION__,  m_tvin_param.port);
    }
...
int CTvin::VDIN_StartDec ( const struct tvin_parm_s *vdinParam )
{
    ...
    if (mSupportResman) {
        if (!Resman_Request(RESMAN_APP_XDVB, RESMAN_ID_AMVIDEO, 2000, NULL)) {
            return -1;
        }
#if RESMAN_SUPPORT_PREEMPT
        char argsize[32] = {0};
        int vdinmmsize = 0;
        struct resman_para res_status;
        char value[PROPERTY_VALUE_MAX] = {0};
        int try_count;
        memset(&res_status, 0, sizeof(res_status));
        res_status.k = RESMAN_ID_CODEC_MM;
        res_status.v.query.type = RESMAN_TYPE_CODEC_MM;
        property_get("media.video.vdinsecure", value, "0");
        res_status.v.query.value = atoi(value);
        property_get("media.video.waitnumber", value, "100");
        try_count  = atoi(value);
        /*vdin playback buffer limit 5185536*10 + di 50M, di may has differnt block count*/
        property_get("media.video.vdinmmsize", value, "94");
        vdinmmsize = atoi(value);
        memcpy(res_status.v.query.name, "vdin", 4);
        sprintf(argsize, "size:%d", vdinmmsize);

        while (Resman_Query((void *)&res_status) >= 0 && try_count > 0)
        {
            if (res_status.v.query.avail > vdinmmsize)
                break;
            usleep(100000);
            try_count --;
        }
        LOGI ( "res_status.v.query.avail 0x%x\n",res_status.v.query.avail);
#endif
        if (!Resman_Request(RESMAN_APP_XDVB, RESMAN_ID_CODEC_MM, 10000, argsize)) {
            return -1;
        }
    } else {
        if ( !isVideoInuse() ) {
    ...
    if ( rt < 0 ) {
        ...
        if (!mSupportResman) {
            ...
            Resman_FreeRes(RESMAN_ID_AMVIDEO);
            Resman_FreeRes(RESMAN_ID_CODEC_MM);
        }
    }
...
int CTvin::VDIN_SetMVCViewMode ( int mode )
{
    char str[32] = {0};
    sprintf (str, "%d", mode);
    tvWriteSysfs(PATH_VDEC_H264MVC_VIEW_MODE, str);
    return 0;
}
...
int CTvin::VDIN_GetMVCViewMode ( void )
{
    ...
    tvReadSysfs(PATH_VDEC_H264MVC_VIEW_MODE, buf);
    int mode = atoi(buf);
    return mode;
}
...
int CTvin::VDIN_SetDIBuffMgrMode ( int mgr_mode )
{
    ...
    tvWriteSysfs(PATH_DI_BUF_MGR_MODE, str);
    return 0;
}
...
int CTvin::VDIN_SetDI3DDetc ( int enable )
{
    char str[32] = {0};
    sprintf (str, "%d", enable);
    tvWriteSysfs(PATH_DI_DET3D_EN, str);
    return 0;
}
...
int CTvin::VDIN_Get3DDetc ( void )
{
    ...
    tvReadSysfs(PATH_DI_DET3D_EN, buf);
...
int CTvin::VDIN_TurnOnBlackBarDetect ( int isEnable )
{
    ...
    tvWriteSysfs(PATH_TVIN_BLACK_BAR_EN, str);
    return 0;
}
...
int CTvin::VDIN_SetDIBypasshd ( int enable )
{
    char str[32] = {0};
    sprintf (str, "%d", enable);
    tvWriteSysfs(PATH_DI_BYPASS_HD, str);
    return 0;
}
...
int CTvin::VDIN_SetDIBypassAll ( int enable )
{
    char str[32] = {0};
    sprintf (str, "%d", enable);
    tvWriteSysfs(PATH_DI_BYPASS_ALL, str);
    return 0;
}
...
int CTvin::VDIN_SetDIBypass_Get_Buf_Threshold ( int enable )
{
    char str[32] = {0};
    sprintf (str, "%d", enable);
    tvWriteSysfs(PATH_DI_BYPASS_GET_BUF_THRESHOLD, str);
    return 0;
}
...
int CTvin::VDIN_SetDIBypassProg ( int enable )
{
    char str[32] = {0};
    sprintf (str, "%d", enable);
    tvWriteSysfs(PATH_DI_BYPASS_PROG, str);
    return 0;
}
...
int CTvin::VDIN_SetDIBypassDynamic ( int flag )
{
    char str[32] = {0};
    sprintf (str, "%d", flag);
    tvWriteSysfs(PATH_DI_BYPASS_DYNAMIC, str);
    return 0;
}
...
int CTvin::TvinApi_SetStartDropFrameCn ( int count )
{
    ...
    int kernelVersion = getKernelMajorVersion();
    if (kernelVersion > 4) {
        return tvWriteSysfs (PATH_DI_START_FRAME_DROP_COUNT_54, set_str );
    } else {
        return tvWriteSysfs (PATH_DI_START_FRAME_DROP_COUNT_49, set_str );
    }
}
...
int CTvin::TvinApi_SetCompPhaseEnable ( int enable )
{
    int ret = -1;
    if ( enable == 1 ) {
        ret = tvWriteSysfs (PATH_AFE_DPHAS_EN_49, "Y" );
        LOGD ( "%s: enable CompPhase.", __FUNCTION__ );
    }
...
int CTvin::TvinApi_GetHDMIAudioStatus ( void )
{
    ...
    int kernelVersion = getKernelMajorVersion();
    if (kernelVersion > 4) {
        tvReadSysfs(PATH_HDMIRX_AUDS_RCV_STS_54, buf);
    } else {
        tvReadSysfs(PATH_HDMIRX_AUDS_RCV_STS_49, buf);
    }
    int val = atoi(buf);
    return val;
}
...
tv_hdmi_port_id_t CTvin::Tvin_GetHdmiPortIDBySourceInput ( tv_source_input_t source_input )
{
    tv_hdmi_port_id_t portId = HDMI_PORT_MAX;
    tvin_port_t port = Tvin_GetSourcePortBySourceInput(source_input);
    switch (port) {
...
int CTvin::Tvin_WaitPathInactive ()
{
    ...
    //try reclaim video resource
    if (mSupportResman) {
      if (Resman_Request(RESMAN_APP_XDVB, RESMAN_ID_AMVIDEO, 2000, NULL)) {
         //actually, we dont need video here,
         //only intent to notice other processes to release video resources
         //and just free after aquire success, and even dont care
         //whether aquire success or not.
         Resman_FreeRes(RESMAN_ID_AMVIDEO);
      }
    }
    for ( i = 0; i<150; i++ ) {
...
int CTvin::Tvin_CheckPathActive ()
{
    ...
    char *str_find = NULL;
    char active_str[] = "(1)";
    char amvideo_str[] = "amvideo"; //video display device
    char vfm_path_flag[] = "["; // use "[" to check if it is video path string
    ...
    while ( fgets ( path, sizeof(path)-1, f ) ) {
        //check if the path is valid, if it's invalid, then ignore it
        if (strstr(path, amvideo_str) && strstr(path, vfm_path_flag)) {
...
int CTvin::Tvin_CheckVideoPathComplete(tv_path_type_t path_type)
{
    ...
    FILE *fp = NULL;
    char path[100] = {0};
    char tvin_str[16] = "tvpath {";
    char *str_find = NULL;
    char decoder_str[16] = "decoder";
    char vdin_str[16] = "vdin0";
    ...
    bool amlvideo2Exist = isFileExist(AMLVIDEO2_DEV_PATH);
    if (amlvideo2Exist) {
        strcpy(amvideo_str, "amlvideo2.0");
    }
    ...
    while (fgets(path, sizeof(path)-1, fp)) {
        ...
        if (str_find != NULL) {
            if ((strstr(str_find, di_str)) &&
                ...
                if (((path_type == TV_PATH_TYPE_DEFAULT) && strstr(str_find, decoder_str))
                  || ((path_type == TV_PATH_TYPE_TVIN) && strstr(str_find, vdin_str))) {
                    ret = 0;
                } else {
                    ret = -1;
                }
...
int CTvin::VDIN_SetMonitorMode(pq_status_update_e mode)
{
    LOGD("%s: monitor mode: %d!\n", __FUNCTION__, mode);
...
int CTvin::VDIN_GetHdmiFormatInfo(tvin_format_s *sourceFormatInfo)
{
    int ret = VDIN_DeviceIOCtl(TVIN_IOC_G_INPUT_TIMING, sourceFormatInfo);
    if (ret < 0) {
        LOGE("%s failed, error(%s)!", __FUNCTION__, strerror(errno));
    }
...
void CTvin::tvin_onResPreemtCallback(void* param)
{
#if RESMAN_SUPPORT_PREEMPT
    LOGE("%s :resmanage onpreemt.", __FUNCTION__);
    CTvin *mTvin = (CTvin*) param;
    if (mTvin->Tvin_getResmanHandle() != -1) {
        ResourceManEvent evt;
        evt.cmd = ResourceManEvent::EVENT_RESMAN_ONPREEMT;
        if (mTvin->mpResObserver)
            mTvin->mpResObserver->onEvent(evt);
    }
#endif
}

int CTvin::Tvin_getResmanHandle()
{
    return mResmanFd;
}

bool CTvin::Resman_Request(int app_type, int res_type, int timeout,const char *arg)
{
    LOGD("%s:tvserver start request resource %d", __FUNCTION__, res_type);
    if (mResmanFd == -1)
        mResmanFd = resman_init("tvserver", app_type);

#if RESMAN_SUPPORT_PREEMPT
    if (!mOnPreemtRegisted) {
         resman_register(mResmanFd, tvin_onResPreemtCallback, this);
         mOnPreemtRegisted = true;
    }
#endif

#if RESMAN_SUPPORT_PREEMPT
    if (!resman_acquire_para(mResmanFd, res_type, timeout, 1, arg)) {
#else
    if (!resman_acquire_wait(mResmanFd, res_type, timeout)) {
#endif
        LOGE("%s:res_auquire(handle=%d,res=%d) failed!!!\n",
         __FUNCTION__,mResmanFd,res_type);
        return false;
    }
    LOGD("%s:tvserver request resource (%d) done ", __FUNCTION__, res_type);

    return true;
}


int CTvin::Resman_Query(void * res_status)
{
    int ret;
    if (mResmanFd == -1)
        return -1;
#if RESMAN_SUPPORT_PREEMPT
    ret = resman_query(mResmanFd, (struct resman_para *) res_status);
#endif
    return ret;
}

void CTvin::Resman_FreeRes(int res_type)
{
    if (mResmanFd == -1)
        return;

    if (resman_release(mResmanFd, res_type)) {
        LOGE("%s:Resman_FreeRes(handle=%d,res=%d) failed!!!\n",
         __FUNCTION__,mResmanFd,res_type);
        return;
    }
}

void CTvin::Resman_Release()
{
    if (mResmanFd != -1) {
#if RESMAN_SUPPORT_PREEMPT
        if (mOnPreemtRegisted) {
            resman_unregister(mResmanFd);
            mOnPreemtRegisted = false;
        }
#endif
        resman_close(mResmanFd);
        mResmanFd = -1;
    }
}

void CTvin::sendEvent(CheckSourceValidEvent &evt)
{
    if (mpResObserver)
