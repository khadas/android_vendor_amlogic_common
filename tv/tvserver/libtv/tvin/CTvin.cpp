...
{
    int ret = -1;
    if (mSupportResman && !Resman_Request(RESMAN_APP_XDVB, RESMAN_ID_VFM_DEFAULT, 2000)) {
        return ret;
    }
...
int CTvin::VDIN_AddVideoPath ( int selPath )
{
    ...
    if (bNeedAcqVfm) {
        if (!Resman_Request(RESMAN_APP_XDVB, RESMAN_ID_VFM_DEFAULT, 2000)) {
            return -1;
        }
    }
...
int CTvin::VDIN_OpenPort ( tvin_port_t port )
{
    ...
    if (!rt && mSupportResman && TVIN_PORT_CVBS0 <= port && port <= TVIN_PORT_CVBS7) {
        if (!Resman_Request(RESMAN_APP_XDVB, RESMAN_ID_ADC_PLL, 2000)) {
...
int CTvin::VDIN_StartDec ( const struct tvin_parm_s *vdinParam )
{
    ...
    if (mSupportResman) {
        if (!Resman_Request(RESMAN_APP_XDVB, RESMAN_ID_AMVIDEO, 2000)) {
            return -1;
        }
    } else {
        if ( !isVideoInuse() ) {
...
bool CTvin::Resman_Request(int app_type, int res_type, int timeout)
{
...
#if RESMAN_SUPPORT_PREEMPT
    if (!resman_acquire_para(mResmanFd, res_type, timeout, 1, NULL)) {
#else
