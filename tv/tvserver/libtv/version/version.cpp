...
static char kernelVersionStr[256] = "N/A";
...
const char *tvservice_get_kernel_version_info(void)
{
    char buf[32] = {0};
    int versionFd = open("/proc/version", O_RDONLY);
    if (versionFd < 0)
        return "Unknow";

    if (read(versionFd, buf, 20)) {
        //Linux version x.x.xx
        close(versionFd);
