...
static const char * const goodix_ts_name[] = {"goodix-ts-lvds", "goodix-ts-mipi", "goodix-ts-hdmi"};

static struct workqueue_struct *goodix_wq[3] = {0};
struct i2c_client *i2c_connect_client[3] = {0};
int gtp_rst_gpio[3] = {0};
int gtp_int_gpio[3] = {0};
...
#define GTP_MAX_KEY_NUM	 (ARRAY_SIZE(touch_key_array) / ARRAY_SIZE(touch_key_array[0]))

#if GTP_DEBUG_ON
static const int  key_codes[] = {KEY_HOME, KEY_BACK, KEY_MENU,
KEY_HOMEPAGE, KEY_F1, KEY_F2, KEY_F3};
static const char * const key_names[] = {"Key_Home", "Key_Back", "Key_Menu"
, "Key_Homepage", "KEY_F1", "KEY_F2", "KEY_F3"};
#endif
#endif
...
s32 gtp_i2c_read(struct i2c_client *client, u8 *buf, s32 len)
{
	...
	if (retries >= 5) {
		...
		gtp_reset_guitar(client, 10, client->flags);
	}
	return ret;
}
...
static void gtp_pen_down(struct goodix_ts_data *ts, s32 x, s32 y, s32 w, s32 id)
{
...
static void gtp_pen_up(struct goodix_ts_data *ts, s32 id)
{
	//struct goodix_ts_data *ts = i2c_get_clientdata(i2c_connect_client);
...
static void goodix_ts_work_func(struct work_struct *work)
{
...
#if GTP_GESTURE_WAKEUP
	if (doze_status == DOZE_ENABLED) {
		ret = gtp_i2c_read(i2c_connect_client[ts->tp_id], doze_buf, 3);
		...
		if (ret > 0) {
			if ((doze_buf[2] == 'a') || (doze_buf[2] == 'b') ||
				...
				// clear 0x814B
				doze_buf[2] = 0x00;
				gtp_i2c_write(i2c_connect_client[ts->tp_id], doze_buf, 3);
	...
	if (key_value || pre_key) {
		...
		if (pre_key == 0x20 || key_value == 0x20 || pre_key == 0x10 ||
			 key_value == 0x10 || key_value == 0x40 || pre_key == 0x40)
			//do nothing
			GTP_INFO("do nothing");
		else
			touch_num = 0;
	...
	if (pre_touch || touch_num) {
		...
		if (touch_num) {
			...
			if ((id & 0x80) && !input_w) {
				...
				gtp_pen_down(ts, input_x, input_y, input_w, 0);
				pre_pen = 1;
				pre_touch = 1;
				pen_active = 1;
			} else if ((id & 0x80) && input_w) {
				if (pre_pen) {
	...
	if (touch_num) {
		for (i = 0; i < touch_num; i++) {
			...
			if ((id & 0x80) && !input_w) {
				GTP_DEBUG("Pen touch DOWN:(%d)(%d, %d)[%d]",
...
static enum hrtimer_restart goodix_ts_timer_handler(struct hrtimer *timer)
{
	...
	queue_work(goodix_wq[ts->tp_id], &ts->work);
 ...
 *	None.
 */
void gtp_int_sync(u8 tp_id, s32 ms)
{
	GTP_GPIO_OUTPUT(gtp_int_gpio[tp_id], 0);
	//msleep(ms);
	usleep_range(10000, 20000);
	GTP_GPIO_AS_INT(gtp_int_gpio[tp_id]);
}
 ...
 *	None.
 */
void gtp_reset_guitar(struct i2c_client *client, s32 ms, u8 tp_id)
{
	GTP_DEBUG_FUNC();
	GTP_INFO("Guitar reset");

	GTP_GPIO_OUTPUT(gtp_rst_gpio[tp_id], 1);
	usleep_range(10000, 20000);

	GTP_GPIO_OUTPUT(gtp_rst_gpio[tp_id], 0);	// begin select I2C slave addr
	//msleep(ms);							// T2: > 10ms
	usleep_range(10000, 20000);
	// HIGH: 0x28/0x29, LOW: 0xBA/0xBB
	GTP_GPIO_OUTPUT(gtp_int_gpio[tp_id], client->addr == 0x14);
	//GTP_GPIO_OUTPUT(gtp_int_gpio[ts->tp_id], 1);
	...
	//	GTP_GPIO_AS_INPUT(gtp_rst_gpio[tp_id]);	// end select I2C slave addr

	gtp_int_sync(tp_id, 50);
#if GTP_ESD_PROTECT
...
static s8 gtp_enter_sleep(struct goodix_ts_data *ts)
{
	...
	GTP_GPIO_OUTPUT(gtp_int_gpio[ts->tp_id], 0);
	//msleep(5);
	usleep_range(10000, 20000);
...
static s8 gtp_wakeup_sleep(struct goodix_ts_data *ts)
{
	...
	while (retry++ < 5) {
		gtp_reset_guitar(ts->client, 20, ts->tp_id);
	...
	while (retry++ < 10) {
		...
		gtp_irq_disable(ts);
		gtp_reset_guitar(ts->client, 10, ts->tp_id);
		gtp_irq_enable(ts);
...
static s32 gtp_init_panel(struct goodix_ts_data *ts)
{
	...
	u8 cfg_info_len[] = {
		ARRAY_SIZE(cfg_info_group0),
		ARRAY_SIZE(cfg_info_group1),
...
static s8 gtp_request_io_port(struct goodix_ts_data *ts)
{
	...
	GTP_DEBUG_FUNC();
	ret = GTP_GPIO_REQUEST(gtp_int_gpio[ts->tp_id], "GTP INT IRQ");
	if (ret < 0) {
		GTP_ERROR("Failed to request INT-GPIO:%d, ERRNO:%d",
		 (s32)gtp_int_gpio[ts->tp_id], ret);
		return -ENODEV;
	}
	GTP_GPIO_AS_INT(gtp_int_gpio[ts->tp_id]);
	ts->client->irq = gpio_to_irq(gtp_int_gpio[ts->tp_id]);

	ret = GTP_GPIO_REQUEST(gtp_rst_gpio[ts->tp_id], "GTP RST PORT");
	if (ret < 0) {
		GTP_ERROR("Failed to request RST-GPIO:%d, ERRNO:%d",
		 (s32)gtp_rst_gpio[ts->tp_id], ret);
		GTP_GPIO_FREE(gtp_int_gpio[ts->tp_id]);
		return -ENODEV;
	}

	GTP_GPIO_AS_INPUT(gtp_rst_gpio[ts->tp_id]);
...
static s8 gtp_request_irq(struct goodix_ts_data *ts)
{
	...
	if (ret) {
		...
		GTP_GPIO_AS_INPUT(gtp_int_gpio[ts->tp_id]);
		GTP_GPIO_FREE(gtp_int_gpio[ts->tp_id]);
...
static s8 gtp_request_input_dev(struct goodix_ts_data *ts)
{
	...
	ts->input_dev->name = goodix_ts_name[ts->tp_id];
	ts->input_dev->phys = goodix_ts_name[ts->tp_id];
	ts->input_dev->uniq = goodix_ts_name[ts->tp_id];
...
#ifdef GTP_CONFIG_OF
/*
 * gtp_parse_dt - parse platform information form devices tree.
 */
static void gtp_parse_dt(struct goodix_ts_data *ts, struct device *dev)
{
	struct device_node *np = dev->of_node;
	u32 temp_val;
	int ret = 0;

	ret = of_property_read_u32(np, "tp_id", &temp_val);
	if (!ret) {
		ts->tp_id = temp_val;
	} else {
		ts->tp_id = 0;
		GTP_ERROR("cannot get tp_id value of touchscreen");
	}

	gtp_int_gpio[ts->tp_id] = of_get_named_gpio(np, "irq-gpio", 0);
	gtp_rst_gpio[ts->tp_id] = of_get_named_gpio(np, "reset-gpio", 0);

	ret = of_property_read_u32(np, "rotation", &temp_val);
	if (!ret) {
		ts->rotation = temp_val;
	} else {
		ts->rotation = 0;
		GTP_ERROR("cannot get rotation value of touchscreen");
	}

	ret = of_property_read_u32(np, "abs_x_max", &temp_val);
	if (!ret)
		ts->abs_x_max = temp_val;
	else
		ts->abs_x_max = GTP_MAX_WIDTH;

	ret = of_property_read_u32(np, "abs_y_max", &temp_val);
	if (!ret)
		ts->abs_y_max = temp_val;
	else
...
static int goodix_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	if (client->dev.of_node)
		gtp_parse_dt(ts, &client->dev);

	GTP_INFO("ts->tp_id=%d", ts->tp_id);

	client->flags = ts->tp_id;
	ret = gtp_i2c_test(client);
	if (ret < 0) {
		GTP_ERROR("I2C communication ERROR!");
		goto err_chip_unconnect;
	}
	...
	gtp_rst_gpio[ts->tp_id] = GTP_RST_PORT;
	gtp_int_gpio[ts->tp_id] = GTP_INT_PORT;
#endif
	INIT_WORK(&ts->work, goodix_ts_work_func);
	ts->client = client;

	i2c_connect_client[ts->tp_id] = client;

	goodix_wq[ts->tp_id] = create_singlethread_workqueue("goodix_wq");
	if (!goodix_wq[ts->tp_id]) {
	...
	GTP_INFO("GTP ts->tp_id = %d", ts->tp_id);
...
err_chip_init:
	GTP_GPIO_FREE(gtp_int_gpio[ts->tp_id]);
	GTP_GPIO_FREE(gtp_rst_gpio[ts->tp_id]);
	//GTP_GPIO_FREE(gtp_int_gpio+3);
	//GTP_GPIO_FREE(gtp_rst_gpio+1);
err_request_io_port:
#if GTP_ESD_PROTECT
	destroy_workqueue(gtp_esd_check_workqueue);
err_create_esd_work:
#endif
	destroy_workqueue(goodix_wq[ts->tp_id]);
...
static void gtp_esd_check_func(struct work_struct *work)
{
	...
	for (i = 0; i < 3; i++) {
		...
		if (ret < 0) {
			...
			if (esd_buf[2] == 0xAA || esd_buf[3] != 0xAA) {
				...
				GTP_DEBUG("[Check]0x8040 = 0x%02X, 0x8041 = 0x%02X",
				 chk_buf[2], chk_buf[3]);
	...
	if (i >= 3) {
		...
		msleep(20);
#endif
		gtp_reset_guitar(ts->client, 50, ts->tp_id);
		msleep(50);
		gtp_send_cfg(ts->client);
	}
