...
/*auto update fw by gtp_default_FW in gt9xx_firmware.h, function together with GTP_AUTO_UPDATE*/
...
/*pen surrport hover or not 1:enable 0 disable*/
#define GTP_WITH_HOVER		  1
...
extern int gtp_rst_gpio[3];
extern int gtp_int_gpio[3];
...
//#define CFG_GROUP_LEN(p_cfg_grp) ((ARRAY_SIZE(p_cfg_grp) / ARRAY_SIZE(p_cfg_grp[0]))
// Log define
#define GTP_INFO(fmt, arg...) pr_info("<<-GT9XX-INFO->> " fmt "\n", ##arg)
#define GTP_ERROR(fmt, arg...) pr_info("<<-GT9XX-ERROR->> " fmt "\n", ##arg)

#define GTP_DEBUG(fmt, arg...) do {\
	if (GTP_DEBUG_ON)\
		pr_info("<<-GT9XX-DEBUG->> [%d]" fmt "\n", __LINE__, ##arg);\
} while (0)
...
#define GTP_DEBUG_FUNC() do {\
	if (GTP_DEBUG_FUNC_ON)\
		pr_info("<<-GT9XX-FUNC->> Func:%s@Line:%d\n", __func__, __LINE__);\
} while (0)
/*End of Part III*/
