...
static unsigned get_dir_filenums(const char * const dirPath)
{
    WIN32_FIND_DATA findFileData;
    HANDLE hdle = INVALID_HANDLE_VALUE;
    int fileNum = 0;
...
static unsigned get_dir_filenums(const char *dir_path)
{
	int count = 0;
	DIR *d;
	struct dirent *de;
                ...
                int needCheckCrc, AmlResImgHead_t* pImgHead, int* imgVersion)
{
        ...
        {//new version magic matched
                ...
                *imgVersion = pImgHead->version;
                ...
                cov_fseek(fdResImg, ImgHeadSz, SEEK_SET, __LINE__);//seek escape the image header to prepare for reading item header
        }
        else
        {
                ...
                *imgVersion = 0;
                cov_fseek(fdResImg, 0, SEEK_SET, __LINE__);//seek escape the image header to prepare for reading item header
        }
...
#if 1
                char* p = itemFullPath;
                strncpy(p, unPackDirPath, MAX_PATH - 2); p[MAX_PATH - 2] = '\0';
                p += strlen(itemFullPath); *p++ = '/';
                strncpy(p, pItemHead->name, MAX_PATH - 1); p[MAX_PATH - 1] = '\0';
#else
...
_exit:
        if (fp_item) fclose(fp_item);
        if (itemReadBuf) free(itemReadBuf), itemReadBuf = NULL;
        ...
        pFunc_getFile getFile, const int totalFileNum)
{
        ...
        for (unsigned itemIndex = 0; itemIndex < totalItemNum; ++itemIndex)
        {
                ...
                strncpy(pItemHeadInfo->name, last_itemname, IH_NMLEN - 1);
                debugP("pack item [%s]\n", last_itemname);
        ...
        if (fseek(fd_dest, 0, SEEK_SET)) {ret = __LINE__; goto _exit;}
        actualWriteLen = fwrite(aAmlResImgHead, 1, HeadLen, fd_dest);
        if (actualWriteLen != HeadLen) {
                errorP("Want to write 0x%x, but 0x%x\n", HeadLen, actualWriteLen);
                ret = __LINE__; goto _exit;
