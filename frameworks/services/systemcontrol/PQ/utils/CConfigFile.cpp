...
#ifdef PRODUCT_SUPPORT_COMPRESS_DB
// convert suffix to *.bin
// example: xxx.txt --> xxx.bin
static inline char * convertToBinSuffix(const char *src, char *dst)
{
    const char *tmp = strchr(src, '.');
    if (tmp != NULL) {
...
void CConfigFile::GetPqdbPath(char *file_path)
{
    if (!isFileExist(PARAM_PQ_DB_PATH)) {
        ...
        char tempPath[128] = {0};
        char pqDBDefaultPath0[128] = PQ_DB_DEFAULT_PATH_0;
        char pqDBDefaultPath1[128] = PQ_DB_DEFAULT_PATH_1;

        Minizip *pMiniz   = NULL;
        if (isFileExist(convertToBinSuffix(pqDBConfigPath, tempPath))
               || isFileExist(convertToBinSuffix(pqDBDefaultPath0, tempPath))
               || isFileExist(convertToBinSuffix(pqDBDefaultPath1, tempPath))) {
            strcpy(pqBinFilePath, tempPath);
        } else {
            SYS_LOGE("no pq.db in %s, %s and %s\n", pqDBConfigPath, pqDBDefaultPath0, pqDBDefaultPath1);
        }
        SYS_LOGD("%s:bin file path: %s\n", __FUNCTION__, pqBinFilePath);
