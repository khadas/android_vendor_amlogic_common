 ...
 */
#ifndef _GAMMAOPEATION_H
#define _GAMMAOPEATION_H
...
// -----------------------
__attribute__((unused)) void spline::set_boundary(spline::bd_type left, double left_value,
...
__attribute__((unused)) void spline::set_points(const std::vector<double>& x,
...
__attribute__((unused)) double spline::operator() (double x) const
{
    size_t n = m_x.size();
...
__attribute__((unused)) double spline::deriv(int order, double x) const
{
    assert(order>0);
...
private:
    static GammaOperation *mInstance;
    //gamma Convert
