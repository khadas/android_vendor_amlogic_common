...
int CPQControl::Cpq_SetDIModuleParam(source_input_param_t source_input_param)
{
    ...
    if (mbCpqCfg_deblock_enable) {
        if (mPQdb->PQ_GetDeblockParams(DI_DEBLOCK_MODE_MIDDLE, source_input_param, &regs) == 0) {
...
int CPQControl::SetCurrentSourceInputInfo(source_input_param_t source_input_param)
{
    ...
    if ((source_input_param.source_input == SOURCE_MPEG)
        ...
        /*SYS_LOGD("%s: signalColorPrimaries = 0x%x, signalTransferCharacteristic = 0x%x, dvFlag = 0x%x.\n",
        ...
        SYS_LOGD("%s: This source no hdr status!\n", __FUNCTION__);
        newHdrType = HDR_TYPE_NONE;
    }

    SYS_LOGD("%s:new source info: source=%d,sigFmt=%d(0x%x),hdrType=%d\n", __FUNCTION__,
