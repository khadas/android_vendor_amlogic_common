...
bool SystemControlService::getActiveDispMode(std::string *activeDispMode) {
    ...
    bool ret = pDisplayMode->getDisplayMode(mode);
    *activeDispMode = mode;
    return ret;
}
...
bool SystemControlService::writeHdcpRX14Key(const char *value, const int size) {
    if (NO_ERROR == permissionCheck()) {
        return pSysWrite->writeHdcpRX14Key(value, size);
    }
    return false;
}
...
bool SystemControlService::writeHdcpRX22Key(const char *value, const int size) {
   if (NO_ERROR == permissionCheck()) {
        return pSysWrite->writeHdcpRX22Key(value, size);
    }
    return false;
}
...
bool SystemControlService::readHdcpRX14Key(const uint32_t key_type, int size) {
    if (NO_ERROR == permissionCheck()) {
        return pSysWrite->readHdcpRX14Key(key_type, size);
    }
    return false;
}
...
bool SystemControlService::readHdcpRX22Key(const uint32_t key_type, int size) {
    if (NO_ERROR == permissionCheck()) {
        return pSysWrite->readHdcpRX22Key(key_type, size);
    }
    return false;
}
...
bool SystemControlService::aisrContrl(bool on) {
    return pDisplayMode->aisrContrl(on);
}

bool SystemControlService::hasAisrFunc() {
    return pDisplayMode->hasAisrFunc();
}

bool SystemControlService::getAisr() {
    return pDisplayMode->getAisr();
}
...
bool SystemControlService::scCpyFile(const std::string &src, const std::string &dest, bool usethread){
    if (usethread) {
        return FileUtils::cpyFileWithThraed(src.c_str(), dest.c_str());
    }
    return FileUtils::cpyFileDirect(src.c_str(), dest.c_str());
}
