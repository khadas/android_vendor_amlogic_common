...
bool SystemControlService::getActiveDispMode(std::string *activeDispMode) {
    ...
    bool ret = DisplayModeMgr::getInstance().getDisplayMode(mode, MODE_LEN);
    *activeDispMode = mode;
    return ret;
}
...
bool SystemControlService::setActiveDispMode(std::string& activeDispMode) {
    if (mLogLevel > LOG_LEVEL_1) {
        ALOGI("set active display mode :%s", activeDispMode.c_str());
    }

    pDisplayMode->setActiveDispMode(activeDispMode.c_str());
    return true;
}
...
bool SystemControlService::readSysfsOri(const std::string& path, std::string& value) {
    if (NO_ERROR == permissionCheck()) {
        char buf[MAX_STR_LEN] = {0};
        Mutex::Autolock _l(mUnifyKeyLock);
        bool ret = pSysWrite->readSysfsOriginal(path.c_str(),buf);
        value = buf;
        traceValue("readSysfsOri", path, value);
        return ret;
    }
    return false;
}
...
bool SystemControlService::writeSysfs(const std::string& path, const std::string& value) {
    if (NO_ERROR == permissionCheck()) {
        ...
        Mutex::Autolock _l(mUnifyKeyLock);
        bool ret = pSysWrite->writeSysfs(path.c_str(), value.c_str());
        return ret;
    }
    return false;
}
...
//Provision key start
bool SystemControlService::writeProvisionKey(const char *value, const int size) {
    bool ret = false;

    if (NO_ERROR == permissionCheck()) {
        ret = pProvisionKey->writeProvisionKey(value, size);
        ALOGD("SystemControlService writeProvisionKey:%d\n", ret);
    }
...
bool SystemControlService::writeUnifyKey(const std::string& path, const std::string& value) {
    if (NO_ERROR == permissionCheck()) {
        ...
        Mutex::Autolock _l(mUnifyKeyLock);
        return pSysWrite->writeUnifyKey(path.c_str(), value.c_str());
    }
    return false;
}

bool SystemControlService::readUnifyKey(const std::string& path, std::string& value) {
    if (NO_ERROR == permissionCheck()) {
        char buf[MAX_STR_LEN] = {0};
        Mutex::Autolock _l(mUnifyKeyLock);
        bool ret = pSysWrite->readUnifyKey(path.c_str(), buf);
        value = buf;

        traceValue("readUnifyKey", path, value);
        return ret;
    }
...
bool SystemControlService::checkProvisionKey(const uint32_t key_type) {
    bool ret = false;

    if (NO_ERROR == permissionCheck()) {
        ret = pProvisionKey->checkProvisionKey(key_type);
        ALOGD("SystemControlService checkProvisionKeyIsExist:%d\n", ret);
    }
...
bool SystemControlService::deleteProvisionKey(const uint32_t key_type) {
    if (NO_ERROR == permissionCheck()) {
        ALOGD("SystemControlService deleteProvisionKey \n");
        return pProvisionKey->deleteProvisionKey(key_type);
    }
    return false;
}

bool SystemControlService::deleteProvisionKeyEx(const uint32_t key_type, const std::string& uuid) {
    bool ret = false;

    if (NO_ERROR == permissionCheck() && uuid.size() < 64) {
        ret = pProvisionKey->deleteProvisionKeyEx(key_type, uuid.c_str());
        ALOGD("SystemControlService deleteProvisionKeyEx:%d\n", ret);
    }
...
bool SystemControlService::writeHdcpRXImg(const std::string& path) {
    if (NO_ERROR == permissionCheck()) {
        ...
        Mutex::Autolock _l(mUnifyKeyLock);
        bool ret = pDisplayMode->writeHdcpRXImg(path.c_str());
        return ret;
    }
...
bool SystemControlService::updataLogoBmp(const std::string& path) {
    if (NO_ERROR == permissionCheck()) {
        traceValue("updataLogoBmp", path, "");
        Mutex::Autolock _l(mUnifyKeyLock);
        bool ret = pDisplayMode->updataLogoBmp(path.c_str());
        return ret;
    }
...
void SystemControlService::setPosition(int left, int top, int width, int height) {
    ...
    pDisplayMode->getDisplayMode(cur_Mode);
    pDisplayMode->setPosition(cur_Mode, left, top, width, height);
}
...
void SystemControlService::setDolbyVisionEnable(int state) {
    pDisplayMode->setDolbyVisionEnable(state, OUTPUT_MODE_STATE_SWITCH);
}

void SystemControlService::setALLMMode(int state) {
    pDisplayMode->setALLMMode(state);
}

bool SystemControlService::isTvSupportALLM() {
    return pDisplayMode->isTvSupportALLM();
}

void SystemControlService::sendHDMIContentType(int state) {
    pDisplayMode->sendHDMIContentType(state);
}

bool SystemControlService::getGameContentTypeSupport() {
    return pDisplayMode->getGameContentTypeSupport();
}

bool SystemControlService::getSupportALLMContentTypeList(std::vector<std::string> *supportModes) {
    return pDisplayMode->getSupportALLMContentTypeList(supportModes);
}
...
void SystemControlService::setPqListener(const sp<PqNotify>& listener) {
    if (pCPQControl != NULL) {
...
void SystemControlService::SendDisplayMode(int mode) {
    if (mNotifyListener != NULL) {
        ALOGI("set displaymode callback\n");
        mNotifyListener->onSetDisplayMode(mode);
    }
}
...
int SystemControlService::getLastPQmode(void)
{
    if (pCPQControl != NULL) {
        return (int)pCPQControl->GetLastPQMode();
    } else {
        return -1;
    }
}
...
int SystemControlService::setSmoothPlusMode(int smoothplus_mode, int is_save)
{
    if (pCPQControl != NULL) {
        return pCPQControl->SetSmoothPlusMode(smoothplus_mode, is_save);
    } else {
        return -1;
    }
}

int SystemControlService::getSmoothPlusMode(void)
{
    if (pCPQControl != NULL) {
        return (int)pCPQControl->GetSmoothPlusMode();
    } else {
        return -1;
    }
}
...
{
    if (pCPQControl != NULL) {
        return pCPQControl->SetHDRTMOMode((hdr_tmo_t)hdrtmo_mode, is_save);
    } else {
        return -1;
    }
}

int SystemControlService::getHDRTMOMode(void)
{
    if (pCPQControl != NULL) {
        return (int)pCPQControl->GetHDRTMOMode();
    } else {
        return -1;
    }
}
...
int SystemControlService::setMemcMode(int memc_mode, int is_save)
{
    if (pCPQControl != NULL) {
        return pCPQControl->SetMemcMode(memc_mode, is_save);
    } else {
        return -1;
    }
}

int SystemControlService::getMemcMode()
{
    if (pCPQControl != NULL) {
        return pCPQControl->GetMemcMode();
    } else {
        return -1;
    }
}

int SystemControlService::setMemcDeBlurLevel(int level, int is_save)
{
    if (pCPQControl != NULL) {
        return pCPQControl->SetMemcDeBlurLevel(level, is_save);
    } else {
        return -1;
    }
}

int SystemControlService::getMemcDeBlurLevel()
{
    if (pCPQControl != NULL) {
        return pCPQControl->GetMemcDeBlurLevel();
    } else {
        return -1;
    }
}

int SystemControlService::setMemcDeJudderLevel(int level, int is_save)
{
    if (pCPQControl != NULL) {
        return pCPQControl->SetMemcDeJudderLevel(level, is_save);
    } else {
        return -1;
    }
}

int SystemControlService::getMemcDeJudderLevel()
{
    if (pCPQControl != NULL) {
        return pCPQControl->GetMemcDeJudderLevel();
    } else {
        return -1;
    }
}
...
int SystemControlService::setDeblockMode(int mode, int isSave)
{
    if (pCPQControl != NULL) {
        return pCPQControl->SetDeblockMode((di_deblock_mode_t)mode, isSave);
    } else {
        return -1;
    }
}

int SystemControlService::getDeblockMode()
{
    if (pCPQControl != NULL) {
        return pCPQControl->GetDeblockMode();
    } else {
        return -1;
    }
}

int SystemControlService::setDemoSquitoMode(int mode, int isSave)
{
    if (pCPQControl != NULL) {
        return pCPQControl->SetDemoSquitoMode((di_demosquito_mode_t)mode, isSave);
    } else {
        return -1;
    }
}

int SystemControlService::getDemoSquitoMode()
{
    if (pCPQControl != NULL) {
        return pCPQControl->GetDemoSquitoMode();
    } else {
        return -1;
    }
}
...
{
    if (pCPQControl != NULL) {
        return pCPQControl->GetSourceHDRType();
    } else {
        return -1;
    }
}
...
tvin_cutwin_t SystemControlService::getOverscanParams(int display_mode)
{
    tvin_cutwin_t cutwin_t;

    if (pCPQControl != NULL) {
        tvin_cutwin_t cutwin_t = pCPQControl->GetOverscanParams((vpp_display_mode_t)display_mode);
        return cutwin_t;
    } else {
        cutwin_t.he = 0;
        cutwin_t.hs = 0;
...
{
    SYS_LOGD("%s:new source info: source=%d,sigFmt=%d(0x%x), transFmt:%d\n", __FUNCTION__,
                                                                     sourceInput,
                                                                     sigFmt,
                                                                     sigFmt,
                                                                     transFmt);
...
int SystemControlService::setCurrentAspectRatioInfo(int aspectRatioInfo)
{
    if (pCPQControl != NULL) {
        return pCPQControl->SetCurrentAspectRatioInfo((tvin_aspect_ratio_e)aspectRatioInfo);
    } else {
        return -1;
    }
}
...
{
    if (pCPQControl != NULL) {
        return pCPQControl->SetColorGamutMode((vpp_colorgamut_mode_t)mode, is_save);
    } else {
        return -1;
    }
}

int SystemControlService::getColorGamutMode(void)
{
    if (pCPQControl != NULL) {
        return pCPQControl->GetColorGamutMode();
    } else {
        return -1;
    }
}
...
void SystemControlService::getChipVersionInfo(std::string& chiversion) {
    char value[MODE_LEN] = {0};
    if (pCPQControl != NULL) {
        pCPQControl->GetChipVersionInfo(value);
    }
...
{
    if (pCPQControl != NULL) {
        return pCPQControl->SetStaticFrameEnable(enable, isSave);
    }
    return -1;
}

int SystemControlService::getStaticFrameEnable()
{
    if (pCPQControl != NULL) {
        return pCPQControl->GetStaticFrameEnable();
    }
    return -1;
}
...
int SystemControlService::getScreenColorForSignalChange()
{
    if (pCPQControl != NULL) {
        return pCPQControl->GetScreenColorForSignalChange();
    }
    return -1;
}

int SystemControlService::setVideoScreenColor(int color)
{
    if (pCPQControl != NULL) {
        return pCPQControl->setVideoScreenColor(color);
    }
    return -1;
}
...
status_t SystemControlService::dump(int fd, const std::vector<std::string>& args) {
    Mutex::Autolock lock(mLock);

    int len = args.size();
    //if there is no any parameters, means dump all informations
    if (0 == len) {
        //default will print all logs
        dprintf(fd, " ------ get all ubootenv data ------ \n");
        pUbootenv->dump(fd);

        char buf[4096] = {0};
        pDisplayMode->dump(buf);
        dprintf(fd, "\n ------ DUMP HDMI display info ------ \n %s", buf);
    }

    for (int i = 0; i < len; i ++) {
        std::string debugLevel("-l");
        std::string bootenv("-b");
        std::string display("-d");
        std::string dimension("-dms");
        ...
        if (args[i] == debugLevel) {
            ...
            if ((i + 3 < len) && (args[i + 1] == std::string("set"))) {
                setBootEnv(args[i+2], args[i+3]);

                dprintf(fd, "set bootenv key:[%s] value:[%s]\n", args[i+2].c_str(), args[i+3].c_str());
                break;
            }
            else if (((i + 2) <= len) && (args[i + 1] == std::string("get"))) {
                if ((i + 2) == len) {
                    dprintf(fd, "get all bootenv, pls see logcat -s SystemControl\n");
                    pUbootenv->dump(fd);
                }
                else {
                    std::string value;
                    ...
                    dprintf(fd, "get bootenv key:[%s] value:[%s]\n", args[i+2].c_str(), value.c_str());
                }
                break;
            }
            else {
                dprintf(fd,
                    ...
                    "lshal debug interface -b [set |get] key value \n");
            }
        }
        else if (args[i] == display) {
            char buf[4096] = {0};
            pDisplayMode->dump(buf);
            dprintf(fd, "HDMI display info: \n %s", buf);
            break;
        }
        else if (args[i] == dimension) {
            char buf[1024] = {0};
            pDimension->dump(buf);
            dprintf(fd, "3D information :\n %s", buf);
            break;
        }
        else if (args[i] == hdcp) {
            pDisplayMode->hdcpSwitch();
            dprintf(fd, "hdcpSwitch for debug hdcp authenticate, have not implemented yet \n");
            break;
        }
        else if (args[i] == help) {
            dprintf(fd,
                "system control hwbinder service use to control the system sysfs property and boot env \n"
                "in multi-user mode, normal process can not get system privilege, so use a transit station \n"
                "usage: \n"
                "lshal debug vendor.amlogic.hardware.systemcontrol@1.0::ISystemControl/default -l value \n"
                "lshal debug vendor.amlogic.hardware.systemcontrol@1.0::ISystemControl/default -b [set |get] key value \n"
                "-l: debug level \n"
                "-b: set or get bootenv \n"
                "-d: dump display mode info \n"
                "-dms: dump 3D information \n"
...
bool SystemControlService::memcContrl(bool on) {
