...
bool SystemControlClient::scCpyFile(const std::string& src,const std::string& dest, bool usethread) {
    return (mSysCtrl->scCpyFile(src, dest, usethread) == Result::OK);
}
...
bool SystemControlClient::frameRateDisplay(bool on, droidlogic::Rect rect) {
    LOG(INFO) << "SystemControlClient frameRateDisplay"<<on;
    ISystemControl::Rect rrect;
    rrect.left = rect.left;
    rrect.right = rect.right;
    rrect.top = rect.top;
    rrect.bottom = rect.bottom;
    Result rtn = mSysCtrl->frameRateDisplay(on,rrect);
    if (rtn == Result::OK) {
        return true;
    }
    return false;
...
bool SystemControlClient::hasAisrFunc() {
    return (mSysCtrl->hasAisrFunc() == Result::OK);
}
...
int SystemControlClient::setMemcMode(int memc_mode, int isSave) {
    return mSysCtrl->setMemcMode(memc_mode, isSave);
}

int SystemControlClient::getMemcMode(void) {
    return mSysCtrl->getMemcMode();
}

int SystemControlClient::setMemcDeBlurLevel(int level, int isSave) {
    return mSysCtrl->setMemcDeBlurLevel(level, isSave);
}

int SystemControlClient::getMemcDeBlurLevel(void) {
    return mSysCtrl->getMemcDeBlurLevel();
}

int SystemControlClient::setMemcDeJudderLevel(int level, int isSave) {
    return mSysCtrl->setMemcDeJudderLevel(level, isSave);
}

int SystemControlClient::getMemcDeJudderLevel(void) {
    return mSysCtrl->getMemcDeJudderLevel();
}
