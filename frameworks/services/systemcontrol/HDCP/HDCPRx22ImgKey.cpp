    ...
    char* datBuf, const unsigned dataBufSz, int* theItemSz)
{
    ...
    if (!imgFileBuf) {
        HDCP_LOGE("Exception: fail to alloc buffer size:%" PRIuFAST16 "\n", fileSize + 1);
        goto _exit3;
    }
    ...
    if (readLen != fileSize) {
        HDCP_LOGE("Fail to read head in sz %zu\n", fileSize);
        goto _exit3;
    }
...
int setImgPath(const char *path)
{
    ...
    if (actualReadSz != IMG_HEAD_SZ) {
        errorP("Want to read %" PRIuFAST16 ", but only read %d\n", IMG_HEAD_SZ, actualReadSz);
        fclose(fdImg);
        free(itemReadBuf);
        return -1;
    }
    ...
    for (num = 0; num < pImgHead->imgItemNum; num++, pItemHead++)
    {
        ...
        if (!strcmp(pItemHead->name, HDCP_RX_PRIVATE)) {
            ...
            if ((access(HDCP_RX_DES_FW_PATH, F_OK)
                  || (getFileSize(HDCP_RX_DES_FW_PATH) != getFileSize(HDCP_FW_LE_OLD_PATH))
