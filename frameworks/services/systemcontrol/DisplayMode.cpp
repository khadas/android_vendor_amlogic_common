...
void DisplayMode::setTvRecoveryDisplay() {
    ...
    DisplayModeMgr::getInstance().getDisplayMode(outputmode, MODE_LEN);
    updateDefaultUI();
...
void DisplayMode::sceneProcess(output_mode_state state) {
    ...
    if (isDolbyVisionEnable() && isTvSupportDolbyVision(tvmode)) {
        if ((state == OUPUT_MODE_STATE_INIT) ||
            (state == OUPUT_MODE_STATE_POWER)) {
            strcpy(mHdmidata.dv_info.dv_cur_displaymode, mHdmidata.ubootenv_hdmimode);
        } else if (OUPUT_MODE_STATE_SWITCH == state) {
            strcpy(mHdmidata.dv_info.dv_cur_displaymode, mHdmidata.ui_hdmimode);
        }

        //1. read dolby vision mode from prop(maybe need to env)
        scene_input_info_t scene_input_info;
        scene_input_info.state                               = (scene_state)state;
        scene_input_info.isbestpolicy                        = isBestOutputmode();

        strcpy(scene_input_info.dv_input_info.ubootenv_dv_type, mHdmidata.dv_info.ubootenv_dv_type);
        ...
        strcpy(scene_input_info.cur_displaymode, mHdmidata.dv_info.dv_cur_displaymode);

        mpSceneProcess->SetSceneInputInfo(&scene_input_info);
        mpSceneProcess->DolbyVisionSceneProcess(&mScene_output_info);

        strcpy(mHdmidata.dv_info.dv_final_displaymode, mScene_output_info.final_displaymode);
        strcpy(mHdmidata.dv_info.dv_final_deepcolor, mScene_output_info.final_deepcolor);
        mHdmidata.dv_info.dv_type = mScene_output_info.dv_type;
    ...
    SYS_LOGI("dv_final_displaymode:%s, dv_final_deepcolor:%s, mHdmidata.dv_info.dv_type:%d\n",
        mHdmidata.dv_info.dv_final_displaymode, mHdmidata.dv_info.dv_final_deepcolor,
        mHdmidata.dv_info.dv_type);

    //3. sdr scene process
    //   and decide final display mode and deepcolor
    if (isDolbyVisionEnable() && isTvSupportDolbyVision(tvmode)) {
        strcpy(mHdmidata.final_displaymode, mHdmidata.dv_info.dv_final_displaymode);
        strcpy(mHdmidata.final_deepcolor, mHdmidata.dv_info.dv_final_deepcolor);
    } else {
        scene_input_info_t scene_input_info;
        scene_input_info.state                               = (scene_state)state;
        scene_input_info.hdmi_input_info.sinkType            = mHdmidata.sinkType;
        scene_input_info.isbestpolicy                        = isBestOutputmode();
        ...
        strcpy(scene_input_info.hdmi_input_info.edidParsing, mHdmidata.edidParsing);
        strcpy(scene_input_info.hdmi_input_info.disp_cap, mHdmidata.disp_cap);
        strcpy(scene_input_info.hdmi_input_info.ubootenv_cvbsmode, mHdmidata.ubootenv_cvbsmode);

        getBootEnv(UBOOTENV_COLORATTRIBUTE, mHdmidata.ubootenv_colorattribute);
        strcpy(scene_input_info.hdmi_input_info.ubootenv_colorattribute, mHdmidata.ubootenv_colorattribute);

        if ((state == OUPUT_MODE_STATE_INIT) ||
            (state == OUPUT_MODE_STATE_POWER)) {
            strcpy(scene_input_info.cur_displaymode, mHdmidata.ubootenv_hdmimode);
        } else if (OUPUT_MODE_STATE_SWITCH == state) {
            strcpy(scene_input_info.cur_displaymode, mHdmidata.ui_hdmimode);
        }

        mpSceneProcess->SetSceneInputInfo(&scene_input_info);
        mpSceneProcess->SDRSceneProcess(&mScene_output_info);

        strcpy(mHdmidata.final_displaymode, mScene_output_info.final_displaymode);
        strcpy(mHdmidata.final_deepcolor, mScene_output_info.final_deepcolor);
    }

    SYS_LOGI("final_displaymode:%s, final_deepcolor:%s\n",
        mHdmidata.final_displaymode, mHdmidata.final_deepcolor);
}
...
void DisplayMode::setSourceDisplay(output_mode_state state) {
    ...
    if ((isHdmiUsed() == true) &&
        ...
        if (isVMXCertification()) {
            DisplayModeMgr::getInstance().setDisplayMode("576cvbs");
        } else {
            DisplayModeMgr::getInstance().setDisplayMode("dummy_l");
        }
    ...
    if ((isHdmiEdidParseOK() == false) &&
        ...
        DisplayModeMgr::getInstance().setDisplayMode(DEFAULT_OUTPUT_MODE);
...
void DisplayMode::setSourceOutputMode(const char* outputmode) {
    output_mode_state state = OUPUT_MODE_STATE_SWITCH;
...
void DisplayMode::applyDisplaySetting(output_mode_state state) {
    ...
    if (isNeedChange) {
        //set hdmi mode
        DisplayModeMgr::getInstance().setDisplayMode(final_displaymode);
    ...
    DisplayModeMgr::getInstance().getDisplayMode(final_Mode, MODE_LEN);
...
void DisplayMode::setSourceOutputMode(const char* outputmode, output_mode_state state) {
    ...
    DisplayModeMgr::getInstance().getDisplayMode(curMode, MODE_LEN);
    ...
    if (strstr(curMode, outputmode) == NULL) {
        if (cvbsMode && (strstr(outputmode, MODE_PANEL) == NULL)) {
            ...
            DisplayModeMgr::getInstance().setDisplayMode("null");
            #endif
        }
        #ifdef HWC_DYNAMIC_SWITCH_VIU
        if (DISPLAY_TYPE_TABLET == mDisplayType &&
                !strcmp("panel", curDisplayMode)) {
            DisplayModeMgr::getInstance().setDisplayMode(outputmode, ConnectorType::CONN_TYPE_CVBS);
        } else {
        ...
        if (isDolbyVisionEnable() && isTvSupportDolbyVision(tmpMode) && strstr(curMode, "i")) {
            ...
            DisplayModeMgr::getInstance().setDisplayMode(highestMode);
        } else {
    ...
    DisplayModeMgr::getInstance().getDisplayMode(finalMode, MODE_LEN);
    if (pSysWrite->getPropertyBoolean(PROP_DISPLAY_SIZE_CHECK, true)) {
        char resolution[MODE_LEN] = {0};
        char defaultResolution[MODE_LEN] = {0};
        char finalResolution[MODE_LEN] = {0};
...
void DisplayMode::getHdmiData(hdmi_data_t* data) {
    ...
    DisplayModeMgr::getInstance().getDisplayMode(data->hdmi_current_mode, MODE_LEN);
...
void DisplayMode::setSinkDisplay(bool initState) {
    ...
    DisplayModeMgr::getInstance().getDisplayMode(current_mode, MODE_LEN);
...
void DisplayMode::getDisplayMode(char* mode) {
    if (mode != NULL) {
        DisplayModeMgr::getInstance().getDisplayMode(mode, MODE_LEN);
        SYS_LOGI("%s mode:%s\n", __FUNCTION__, mode);
    } else {
...
void DisplayMode::enableDolbyVision(int DvMode) {
    ...
    if (DISPLAY_TYPE_TV == mDisplayType) {
        ...
        DisplayModeMgr::getInstance().setDisplayAttribute(DISPLAY_DOLBY_VISION_POLICY, DV_POLICY_FOLLOW_SOURCE, ConnectorType::CONN_TYPE_HDMI);
    }
...
void DisplayMode::setDolbyVisionEnable(int state,  output_mode_state mode_state) {
    ...
    SYS_LOGI("dolby vision mode:%d", state);
    //1. update prop
    sprintf(tmp, "%d", state);
    ...
    DisplayModeMgr::getInstance().getDisplayMode(cur_displaymode, MODE_LEN);
        ...
        if (!(strstr(dv_cap.c_str(), "The Rx don't support DolbyVision")) &&
            ...
            if (!strcmp(hdr_policy, HDR_POLICY_SOURCE)) {
                ...
                if (strstr(dv_cap.c_str(), "2160p60hz")) {
                    ...
                    setBootEnv(UBOOTENV_OUTPUTMODE, "2160p60hz");
