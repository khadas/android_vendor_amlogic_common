...
SoftWareFormatCovert::SoftWareFormatCovert() {
    ALOGI("SoftWareFormatCovert construct ");
}

SoftWareFormatCovert::~SoftWareFormatCovert() {
    ALOGI("~SoftWareFormatCovert");
}
...
bool HardWareFormatCovert::covert(const char* src_buff,unsigned int src_fmt, size_t src_w, size_t src_h,
                            char* dst_buff,unsigned int dst_fmt, size_t dst_w, size_t dst_h) {
    bool ret = false;
    int32_t src_size = -1,dst_size = -1;
    int32_t src_dma_fd = -1,dst_dma_fd = -1;
    char* input = nullptr;
    char* output = nullptr;
    int64_t first_times = getNowTimesUs();
    if (!src_buff || !dst_buff) {
    ...
    if (src_size <= 0 ) {
        ALOGE("[%s %d]  get the size of src format src_fmt:%d (%d:%d) ", __FUNCTION__, __LINE__,
                src_fmt, src_w, src_h);
        goto exit;
    }
    ...
    ALOGD("[%s %d]  alloc memory src dma buffer fd = %d, size = %d ", __FUNCTION__, __LINE__,
            src_dma_fd, src_size);
    dst_size = getBufferSize(dst_fmt, dst_w, dst_h);
     if (dst_size <= 0 ) {
        ALOGE("[%s %d]  get the size of dst format dst_fmt:%d (%d:%d) ", __FUNCTION__, __LINE__,
                dst_fmt, dst_w, dst_h);
        goto exit;
    }
    ...
    if (ge2DFmtConvert(dst_dma_fd, getPixFormat(dst_fmt),dst_w, dst_h,src_dma_fd, getPixFormat(src_fmt), src_w, src_h) < 0) {
        goto exit3;
    }
    output  = (char*)mmap(NULL, dst_size,
