...
static const char* FILE_TYPE_STR_ARR[] = {"BMP", "BINARY"};
...
int main(int argc, char** argv) {
    ...
    printf(
        "type=%d(%s), file type:%s\n"
        "(left,top,right,bottom)=(%d,%d,%d,%d)\n"
        "out(width,height)=(%d,%d)\n"
        "counter=%d\n",
        type, CAPTURE_TYPE_STR_ARR[type], FILE_TYPE_STR_ARR[saveFileType], left, top, right, bottom, outWidth,
        outHeight, counter);
    while (1) {
        ...
        int ret = client->startScreenCapBuffer(left, top, right, bottom, outWidth, outHeight, type, (void**)&buffer,
                                               &bufferSize);
        ...
        if (isSaveFile) {
            ...
            if (saveFileType == SAVE_FILE_BMP) {
                ...
                snprintf(dump_path, 128, "%s/%s-%dx%d-%d.bin", dump_dir, "argb8888", outWidth, outHeight, nowCounter);
            }
            printf("Try save:%s\n", dump_path);
