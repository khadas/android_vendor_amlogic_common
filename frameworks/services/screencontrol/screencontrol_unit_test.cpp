...
public:
    ...
    bool start(int left, int top,int right,int bottom,int width,int height,int source_type,
                int32_t frame_rate,int32_t bit_rate) {
...
TEST_F(ScreenControlUnitTest, testLoopAvcRecord) {
    ...
    for (int i = 0;i < RUN_COUNT;i++) {
        sp<AvcRecorder> recorder = new AvcRecorder();
...
TEST_F(ScreenControlUnitTest, testLoopYuvRecord) {
    ...
    for (int i = 0;i < RUN_COUNT;i++) {
        sp<YuvRecorder> recorder = new YuvRecorder();
