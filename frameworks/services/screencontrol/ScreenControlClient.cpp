...
int ScreenControlClient::startScreenCapBuffer(int32_t left, int32_t top, int32_t right, int32_t bottom, int32_t width,
                                              int32_t height, int32_t sourceType, void** buffer, int* bufSize) {
    Mutex::Autolock autoLock(mScreenCapLock);
    int result = -1;
    ALOGI("enter %s,left=%d,top=%d,right=%d,bottom=%d,width=%d,height=%d,srctype=%d", __func__, left, top, right,
          bottom, width, height, sourceType);
...
int32_t ScreenControlClient::startScreenCapBuffer(int32_t width, int32_t height, int32_t sourceType,
...
int ScreenControlClient::startScreenRecord(int32_t left, int32_t top, int32_t right, int32_t bottom, int32_t width,
                                           int32_t height, int32_t frameRate, int32_t bitRate, int32_t limitTimeSec,
                                           int32_t sourceType, const char* filename) {
    Mutex::Autolock autoLock(mLock);
    int result = -1;
    ALOGI(
        "enter %s,left=%d,top=%d,right=%d,bottom=%d, width=%d,height=%d,rate=%d,bitrate=%d,timesec=%d,srctype=%d,filename=%s",
        __func__, left, top, right, bottom, width, height, frameRate, bitRate, limitTimeSec, sourceType, filename);
    if (Result::OK == mScreenCtrl->startScreenRecord(left, top, right, bottom, width, height, frameRate, bitRate,
                                                     limitTimeSec, sourceType, filename))
        result = 0;
    return result;
}

int ScreenControlClient::startScreenRecord(int32_t width, int32_t height, int32_t frameRate, int32_t bitRate,
                                           int32_t limitTimeSec, int32_t sourceType, const char* filename) {
    return startScreenRecord(0, 0, width, height, width, height, frameRate, bitRate, limitTimeSec, sourceType,
                             filename);
}
...
int ScreenControlClient::startAvcScreenRecord(int32_t left, int32_t top, int32_t right, int32_t bottom, int32_t width,
    ...
    ALOGI("enter %s,left=%d,top=%d,right=%d,bottom=%d, width=%d,height=%d,rate=%d,bitrate=%d,srctype=%d", __func__,
          left, top, right, bottom, width, height, frameRate, bitRate, sourceType);
    if (Result::OK ==
        mScreenCtrl->startAvcRecord(left, top, right, bottom, width, height, frameRate, bitRate, sourceType))
        result = 0;
    return result;
}

int ScreenControlClient::startAvcScreenRecord(int32_t width, int32_t height, int32_t frameRate, int32_t bitRate,
                                              int32_t sourceType) {
...
int ScreenControlClient::startYuvScreenRecord(int32_t left, int32_t top, int32_t right, int32_t bottom, int32_t width,
    ...
    ALOGI("enter %s,left=%d,top=%d,right=%d,bottom=%d, width=%d,height=%d,rate=%d,srctype=%d", __func__, left, top,
          right, bottom, width, height, frameRate, sourceType);
    if (Result::OK == mScreenCtrl->startYuvRecord(left, top, right, bottom, width, height, frameRate, sourceType))
        result = 0;
    return result;
}
...
Return<void> ScreenControlClient::ScreenControlHidlCallback::onAvcDataArouse(const hidl_memory& mem, int32_t size,
                                                                             int32_t frame_type, int64_t pts) {
