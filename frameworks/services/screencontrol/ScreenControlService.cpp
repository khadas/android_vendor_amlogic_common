...
sp<ScreenControlService>& ScreenControlService::getInstance() {
    static sp<ScreenControlService> mScreenControl = new ScreenControlService();
...
int32_t ScreenControlService::startScreenCapBuffer(int32_t left, int32_t top, int32_t right, int32_t bottom,
                                                   int32_t width, int32_t height, int32_t sourceType, void* dstBuffer,
                                                   int32_t* dstBufferSize) {
    ALOGI("[%s] left:%d, top:%d, right:%d, bottom:%d, width:%d, height:%d, sourceType:%d\n", __func__, left, top, right,
...
int32_t ScreenControlService::startScreenRecord(int32_t left, int32_t top, int32_t right, int32_t bottom, int32_t width,
                                                int32_t height, int32_t frameRate, int32_t bitRate,
                                                int32_t limitTimeSec, int32_t sourceType, const char* filename) {
    ALOGI(
        "[%s] left:%d, top:%d, right:%d, bottom:%d, width:%d, height:%d, sourceType:%d,frameRate=%d,bitRate=%d,limitTimeSec=%d",
...
int32_t ScreenControlService::startAvcRecord(int32_t left, int32_t top, int32_t right, int32_t bottom, int32_t width,
...
int32_t ScreenControlService::startYuvRecord(int32_t left, int32_t top, int32_t right, int32_t bottom, int32_t width,
                                             int32_t height, int32_t frameRate, int32_t sourceType) {
    ALOGI("[%s] left:%d, top:%d, right:%d, bottom:%d, width:%d, height:%d, sourceType:%d,frameRate=%d", __func__, left,
          top, right, bottom, width, height, sourceType, frameRate);
    std::lock_guard<std::mutex> alock(mLock);
    std::unique_lock<std::mutex> sl(mScreenCapLock);
    if (mScreenCatch) {
        mWaitStop = true;
...
void ScreenControlService::PictureReady(const OutputRecord& output) {
    ...
    if (id == 0) {
        if (mScreenManager)
            mScreenManager->releaseBuffer(id, output.index);
    } else
