...
status_t TSPacker::setMaxFrameCount(int32_t maxFrameCnt) {
    ALOGD("setMaxFrameCount(), maxFrameCnt=%d", maxFrameCnt);
    Mutex::Autolock lock(mMutex);
    mMaxFrameCnt = maxFrameCnt;
    return OK;
}

int32_t TSPacker::getMaxFrameCount() const {
    ALOGD("getMaxFrameCount()=%d", mMaxFrameCnt);
    Mutex::Autolock lock(mMutex);
    return mMaxFrameCnt;
}

status_t TSPacker::setTimeLimit(int32_t timeLimitMs) {
    ALOGD("setTimeLimit(), timeLimitMs=%d", timeLimitMs);
    Mutex::Autolock lock(mMutex);
...
int TSPacker::threadFunc()
{
    ...
    while (mStarted == true) {
        ...
        ALOGI("[%s %d] csd buffer size:%d", __FUNCTION__, __LINE__, tESBuffer->range_length());
...
void TSPacker::headFinalize() {
...

    {
        ...
        ALOGI("[%s %d] make AVC video descriptor, size is 6",__FUNCTION__, __LINE__);
...

    {
        ...
        // reserved = 11111b
        data[3] = 0x1f;
        ALOGI("[%s %d] make AVC timing and HRD descriptor, size is 4",__FUNCTION__, __LINE__);
        mDescriptors.push_back(descriptor);
    }
    ...
    if (mHasAudio && mIsPcmAudio) {
        ...
        ALOGI("[%s %d] make  LPCM audio stream descriptor, size is 4",__FUNCTION__, __LINE__);
...
status_t TSPacker::checkConvertDone() {
    return mVideoConvertor->checkConvertDone();
}
