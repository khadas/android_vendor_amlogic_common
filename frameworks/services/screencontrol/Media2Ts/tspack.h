...
struct TSBufferInfo {
    TSBufferInfo(uint8_t* buffer, int32_t size, int64_t pts)
            : mTsbuffer(buffer),
              mSize(size),
              mPts(pts) {};
...
private:
    bool packetize(const uint8_t* es_buffer, int32_t es_size, uint8_t** packets, int32_t* ts_size, int64_t timeUs,
                   uint32_t flags);
