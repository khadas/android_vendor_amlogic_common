...
public:
    ...
    bool Contains(const Area& area) const {
        return (area.x() >= x() && area.right() <= right() && area.y() >= y() && area.bottom() <= bottom());
    }
...
inline bool operator!=(const Area& lhs, const Area& rhs) { return !(lhs == rhs); }
